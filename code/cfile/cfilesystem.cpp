...
int cf_get_packfile_count(cf_root *root)
{
	...
	for (i=CF_TYPE_ROOT; i<CF_MAX_PATH_TYPES; i++ )
	{
		...
		{
			filespec += Pathtypes[ i ].path.string();
			if ( filespec[ filespec.length( ) - 1 ] != core::fs::path::preferred_separator )
			{
				filespec += core::fs::path::preferred_separator;
			}
		}
...
void cf_build_pack_list( cf_root *root )
{
	...
	for (i = CF_TYPE_ROOT; i < CF_MAX_PATH_TYPES; i++) {
		...
		if ( !(Pathtypes[i].path.empty()) ) {
			strcat_s( filespec, Pathtypes[i].path.c_str() );

			if ( filespec[strlen(filespec)-1] != core::fs::path::preferred_separator )
			...
			do {
				...
				if (!(find.attrib & _A_SUBDIR)) {					
					...
					if(!(Pathtypes[i].path.empty())) {
						...
						strcat_s(rptr_sort->path, Pathtypes[i].path.c_str() );
						strcat_s(rptr_sort->path, core::fs::preferred_separator_string);
					}
...
static char normalize_directory_separator(char in)
{
	if (in == '/')
	{
		return core::fs::path::preferred_separator;
	}
...
static void cf_add_mod_roots(const char* rootDirectory, uint32_t basic_location)
{
	if (Cmdline_mod)
	{
		...
		for (const char* cur_pos=Cmdline_mod; strlen(cur_pos) != 0; cur_pos+= (strlen(cur_pos)+1))
		{
			...
			if (rootDirectory[strlen(rootDirectory) - 1] != core::fs::path::preferred_separator)
			{
				ss << core::fs::path::preferred_separator;
			}

			ss << cur_pos << core::fs::path::preferred_separator;
...
void cf_build_root_list(const char *cdrom_dir)
{
	...
	if (os_is_legacy_mode())
	{
		...
		if ((strlen(root->path) < (CF_MAX_PATHNAME_LENGTH - 1)) && (root->path[strlen(root->path) - 1] != core::fs::path::preferred_separator)) {
			strcat_s(root->path, core::fs::path::preferred_separator);		// put trailing backslash on for easier path construction
		}
		root->roottype = CF_ROOTTYPE_PATH;
	...
	else if (!Cmdline_portable_mode)
	{
		...
		// do we already have a slash? as in the case of a root directory install
		if ((strlen(root->path) < (CF_MAX_PATHNAME_LENGTH - 1)) && (root->path[strlen(root->path) - 1] != core::fs::path::preferred_separator)) {
			strcat_s(root->path, core::fs::preferred_separator_string);		// put trailing backslash on for easier path construction
		}
		root->roottype = CF_ROOTTYPE_PATH;
	...
	// do we already have a slash? as in the case of a root directory install
	if ( (path_len < (CF_MAX_PATHNAME_LENGTH-1)) && (root->path[path_len-1] != core::fs::path::preferred_separator) ) {
		strcat_s(root->path, core::fs::preferred_separator_string);		// put trailing backslash on for easier path construction
	}
...
void cf_search_root_path(int root_index)
{
	...
	for (i=CF_TYPE_ROOT; i<CF_MAX_PATH_TYPES; i++ )	{
		...
		if(!(Pathtypes[i].path.empty())) {
			strcat_s( search_path, Pathtypes[i].path.c_str() );
			if ( search_path[strlen(search_path)-1] != core::fs::path::preferred_separator ) {
				strcat_s( search_path, core::fs::preferred_separator_string );
			}
		} 
				...
				if (!(find.attrib & _A_SUBDIR)) {
					...
					if ( ext )	{
						if ( is_ext_in_list( Pathtypes[i].extensions, ext ) )	{
							...
							std::string file_name = (core::fs::path(search_directory) / find.name).string();
	...
	for (i=0; i<VP_header.num_files; i++ )	{
		...
		if ( find.size == 0 )	{
			...
			if ( !stricmp( find.filename, ".." ))	{
				...
				while( (p > search_path) && (*p != core::fs::path::preferred_separator) )	{
					p--;
				}
				*p = 0;
			} else {
				if ( search_path_len && (search_path[search_path_len-1] != core::fs::path::preferred_separator) ) {
					strcat_s( search_path, core::fs::preferred_separator_string );
				}
...
CFileLocation cf_find_file_location(const char* filespec, int pathtype, bool localize, uint32_t location_flags)
{
	...
	for (ui = 0; ui < Num_files; ui++ )	{
			...
			if ( lcl_add_dir_to_path_with_filename(longname, MAX_PATH_LEN - 1) ) {
				if ( !stricmp(longname, f->name_ext) ) {
					...
					if (f->data != nullptr) {
						...
						res.full_name = Pathtypes[f->pathtype_index].path.string();
						res.full_name += core::fs::path::preferred_separator;
		...
		if ( filespec && !stricmp(filespec, f->name_ext) ) {
			CFileLocation res(true);
			res.size = static_cast<size_t>(f->size);
			res.offset = (size_t)f->pack_offset;
			res.data_ptr = f->data;
                                  ...
                                  uint32_t location_flags)
{
	...
	if ( filename && strpbrk(filename,"/\\:")  ) {
		...
		strcat_s(path, path_max, Pathtypes[pathtype].path.c_str());

		// Don't add slash for root directory
		if (Pathtypes[pathtype].path.c_str()[0] != '\0') {
			if ( path[strlen(path)-1] != core::fs::path::preferred_separator ) {
				strcat_s(path, path_max, core::fs::preferred_separator_string);
			}
		}
                                  ...
                                  uint32_t location_flags)
{
	...
	if ( filename && strpbrk(filename,"/\\:")  ) {
		...
		s_path << Pathtypes[pathtype].path.string();

		// Don't add slash for root directory
		if (Pathtypes[pathtype].path.c_str()[0] != '\0') {
			if ( *(s_path.str().rbegin()) != core::fs::path::preferred_separator ) {
				s_path << core::fs::path::preferred_separator;
			}
		//	if ( path[strlen(path)-1] != core::fs::path::preferred_separator ) {
		//		strcat_s(path, path_max, core::fs::path::preferred_separator);
		//	}
		}
