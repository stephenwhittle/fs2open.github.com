...
//but honestly thats a limitation I can live with for now. Worst case change the signature on DeserializeToField to take in an instance
//which is then captured by reference, but I think this is safer
template<typename T>
...
//can create other functions instead so long as they maintain the void(ClassType* ClassInstance, const SCPParsedTableData& InData) signature
...
{
public:
	SCPVersionInfo(int Major, tl::optional<int> Minor = tl::nullopt, tl::optional<int> Build = tl::nullopt, tl::optional<int> Revision = tl::nullopt)
		: Major(Major),
		Minor(Minor),
		Build(Build),
		Revision(Revision)
