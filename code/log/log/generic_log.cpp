...
bool logfile_init(int logfile_type)
{
	if((logfile_type < 0) || (logfile_type >= MAX_LOGFILES)) {
		GOutputDevice->Warning(LOCATION, "Attempt to write illegal logfile number %d", logfile_type);
		return false;
	}
...
void log_string(int LogType, const char* Message, int Time /*= 0*/)
{
	...
	cfputs(tmp, logfiles[LogType].log_file);
	cflush(logfiles[LogType].log_file);
