...
debug_command::debug_command(const char *_name, const char *_help, void(*_func)())
	...
	if (dc_commands_size >= DC_MAX_COMMANDS) {
		GOutputDevice->Error(LOCATION, "Too many debug console commands! Please inform a coder to increase DC_MAX_COMMANDS.");
		return;
	}
	...
	for (; i < dc_commands_size; ++i) {
		...
		if (ret == 0) {
			GOutputDevice->Error(LOCATION, "Debug Command %s already exists! Please inform a coder immediately.", _name);
			return;
		} else if (ret > 0) {
