...
ADE_FUNC(__gc, l_ModelPath, nullptr, "Removes the allocated reference of this handle", nullptr, nullptr)
{
	model_path_h* p = nullptr;
	if (!ade_get_args(L, "o", l_ModelPath.Get(&p)))
            ...
            "The point or invalid handle if index is invalid")
{
	model_path_h* p = nullptr;
	int idx;
	if (!ade_get_args(L, "oi", l_ModelPath.Get(&p), &idx))
