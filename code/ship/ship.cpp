...
#define NUM_SHIP_SUBSYSTEMS_PER_SET                                                                                    \
	200 // Reduced from 1000 to 400 by MK on 4/1/98.  DTP; bumped from 700 to 2100
...
flag_def_list_new<Thruster_Flags> Man_types[] = {
    {"Bank right", Thruster_Flags::Bank_right, true, false},   {"Bank left", Thruster_Flags::Bank_left, true, false},
    {"Pitch up", Thruster_Flags::Pitch_up, true, false},       {"Pitch down", Thruster_Flags::Pitch_down, true, false},
    {"Roll right", Thruster_Flags::Roll_right, true, false},   {"Roll left", Thruster_Flags::Roll_left, true, false},
    {"Slide right", Thruster_Flags::Slide_right, true, false}, {"Slide left", Thruster_Flags::Slide_left, true, false},
    {"Slide up", Thruster_Flags::Slide_up, true, false},       {"Slide down", Thruster_Flags::Slide_down, true, false},
    {"Forward", Thruster_Flags::Forward, true, false},         {"Reverse", Thruster_Flags::Reverse, true, false}};
...
flag_def_list_new<Weapon::Info_Flags> ai_tgt_weapon_flags[] = {
    ...
    //{ "beam",						Weapon::Info_Flags::Beam,								true, false },	// Okay, this one probably doesn't
    //make sense.
...
static int ship_obj_list_add(int objnum)
{
	...
	if (i == MAX_SHIP_OBJS) {
		core::Error(LOCATION, "Fatal Error: Ran out of ship object nodes\n");
		return -1;
	}
...
static void parse_engine_wash(bool replace)
{
	...
	if (optional_string("+nocreate")) {
		if (!replace) {
			core::Warning(LOCATION, "+nocreate flag used for engine wash in non-modular table");
		}
...
void ship_info::clone(const ship_info& other)
{
	...
	if (other.n_subsystems > 0) {
			...
			for (int i = 0; i < n_subsystems; i++) {
				for (int j = 0; j < subsystems[i].n_triggers; j++) {
					if (i < other.n_subsystems) {
						subsystems[i].triggers = (queued_animation*)vm_realloc(
						    subsystems[i].triggers, sizeof(queued_animation) * (other.subsystems[i].n_triggers));
					} else {
		...
		for (int i = 0; i < other.n_subsystems; i++) {
			...
			memcpy(subsystems[i].triggers, other.subsystems[i].triggers,
			       sizeof(queued_animation) * (subsystems[i].n_triggers));
		}
	}
	...
	// ...Hmm. A memcpy() seems slightly overkill here, but I've settled into the pattern of "array gets memcpy'd",
	// so... -MageKing17
	...
	memcpy(allowed_bank_restricted_weapons, other.allowed_bank_restricted_weapons,
...
static int parse_ship(const char* filename, bool replace)
{
	...
	if (optional_string("+nocreate")) {
		if (!replace) {
			core::Warning(LOCATION, "+nocreate flag used for ship in non-modular table");
		}
		create_if_not_found = false;
	}
	...
	if (ship_id != -1) {
		...
		if (!replace) {
			core::Warning(LOCATION, "Error:  Ship name %s already exists in %s.  All ship class names must be unique.",
			        sip->name, filename);
			if (!skip_to_start_of_string_either("$Name:", "#End")) {
				Int3();
			}
		...
		if (Ship_info.size() >= MAX_SHIP_CLASSES) {
			core::Error(LOCATION, "Too many ship classes before '%s'; maximum is %d.\n", buf, MAX_SHIP_CLASSES);
		}
		...
		if (!create_if_not_found) {
			...
			if (template_id != -1) {
				...
				core::Warning(LOCATION,
				        "Unable to find ship template '%s' requested by ship class '%s', ignoring template request...",
				        template_name, buf);
			}
		}
	}
...
static int parse_ship_template()
{
	...
	if (template_id != -1) {
		...
		core::Warning(LOCATION, "WARNING: Ship template %s already exists. All ship template names must be unique.",
		        sip->name);
...
static void parse_allowed_weapons(ship_info* sip, const bool is_primary, const bool is_dogfight, const bool first_time)
{
	...
	if (optional_string(allowed_banks_str)) {
		...
		while (check_for_string("(")) {
			...
			if (bank >= max_banks) {
				core::Warning(LOCATION,
				        "%s bank-specific loadout for %s exceeds permissible number of %s banks.  Ignoring the rest...",
			...
			for (i = 0; i < num_allowed; i++) {
				if (allowed_weapons[i] >=
				    0) // MK, Bug fix, 9/6/99.  Used to be "allowed_weapons" not "allowed_weapons[i]".
				{
...
/**
 * Common method for parsing ship/subsystem primary/secondary weapons so that the parser doesn't flip out in the event
 * of a problem.
 *
 */
                              ...
                              int* bank_capacities)
{
	...
	if ((num_banks != NULL) && (*num_banks != num_bank_capacities)) {
		...
		if (is_primary && num_bank_capacities != 0) {
			core::Warning(LOCATION, "Ship class '%s' has %d primary banks, but %d primary capacities... fix this!!",
			        sip->name, *num_banks, num_bank_capacities);
		}

		// secondaries have no excuse!
		if (!is_primary) {
			core::Warning(LOCATION, "Ship class '%s' has %d secondary banks, but %d secondary capacities... fix this!!",
			        sip->name, *num_banks, num_bank_capacities);
		}
	}
}
 ...
 *
 * If we are creating a ship, we want to inherit the parameters of the ship class, then override on a field-by-field
 * basis.
 */
                      ...
                      const char* info_name)
{
	...
	if (optional_string(str)) {
		...
		if (j >= 0) {
			...
			if (j >= 0) {
				...
				core::Warning(LOCATION, "Invalid %s '%s' specified for %s '%s'", str, buf, info_type_name, info_name);
	...
	if (direction == WarpDirection::WARP_OUT) {
		...
		if (optional_string(str)) {
			...
			if (t_time > 0.0f)
				...
				core::Warning(LOCATION, "%s specified as 0 or less on %s '%s'; value ignored", str, info_type_name,
				        info_name);
		}
	}
	...
	if (optional_string(str)) {
		...
		if (speed > 0.0f)
			...
			core::Warning(LOCATION, "%s specified as 0 or less on %s '%s'; value ignored", str, info_type_name, info_name);
	}
	...
	if (optional_string(str)) {
		...
		if (accel_exp >= 0.0f)
			...
			core::Warning(LOCATION, "%s specified as less than 0 on %s '%s'; value ignored", str, info_type_name, info_name);
	}
...
static int parse_ship_values(ship_info* sip, const bool is_template, const bool first_time, const bool replace)
{
	...
	if (optional_string("$Species:")) {
		...
		if (!found) {
			core::Error(LOCATION, "Invalid Species %s defined in table entry for ship %s.\n", temp, sip->name);
		}
	}
	...
	if (optional_string("$Cockpit POF file:")) {
		...
		// Goober5000 - if this is a modular table, and we're replacing an existing file name, and the file doesn't
		// exist, don't replace it
		if (replace)
	...
	while (optional_string("$Cockpit Display:")) {
		...
		if (display.offset[0] < 0 || display.offset[1] < 0) {
			core::Warning(LOCATION, "Negative display offsets given for cockpit display on %s, skipping entry", sip->name);
			continue;
		}

		if (display.size[0] <= 0 || display.size[1] <= 0) {
			core::Warning(LOCATION, "Negative or zero display size given for cockpit display on %s, skipping entry",
			        sip->name);
			continue;
		}
	...
	if (optional_string("$POF file Techroom:")) {
		...
		// if this is a modular table, and we're replacing an existing file name, and the file doesn't exist, don't
		// replace it
		if (replace)
	...
	if (optional_string("$Default Team:")) {
		...
		if (!stricmp(temp, "none")) {
			...
			if (Team_Colors.find(name) != Team_Colors.end()) {
				...
				core::Warning(LOCATION, "Team name %s is invalid. Teams must be defined in colors.tbl.\n", temp);
			}
		}
	}
	...
	if (optional_string("$Damage Lightning Type:")) {
		...
		if (j >= 0) {
			...
			core::Warning(LOCATION, "Invalid lightning type '%s' specified for ship '%s'", buf, sip->name);
	...
	if (optional_string("$Debris:")) {
		if (optional_string("+Min Lifetime:")) {
			...
			if (sip->debris_min_lifetime < 0.0f)
				core::Warning(LOCATION, "Debris min lifetime on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
		if (optional_string("+Max Lifetime:")) {
			stuff_float(&sip->debris_max_lifetime);
			if (sip->debris_max_lifetime < 0.0f)
				core::Warning(LOCATION, "Debris max lifetime on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
		if (optional_string("+Min Speed:")) {
			stuff_float(&sip->debris_min_speed);
			if (sip->debris_min_speed < 0.0f)
				core::Warning(LOCATION, "Debris min speed on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
		if (optional_string("+Max Speed:")) {
			stuff_float(&sip->debris_max_speed);
			if (sip->debris_max_speed < 0.0f)
				core::Warning(LOCATION, "Debris max speed on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
		...
		if (optional_string("+Min Hitpoints:")) {
			...
			if (sip->debris_min_hitpoints < 0.0f)
				core::Warning(LOCATION, "Debris min hitpoints on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
		if (optional_string("+Max Hitpoints:")) {
			stuff_float(&sip->debris_max_hitpoints);
			if (sip->debris_max_hitpoints < 0.0f)
				core::Warning(LOCATION, "Debris max hitpoints on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
		if (optional_string("+Damage Multiplier:")) {
			stuff_float(&sip->debris_damage_mult);
			if (sip->debris_damage_mult < 0.0f)
				core::Warning(LOCATION, "Debris damage multiplier on %s '%s' is below 0 and will be ignored", info_type_name,
				        sip->name);
		}
	...
	if (sip->debris_min_speed > sip->debris_max_speed && sip->debris_max_speed >= 0.0f) {
		core::Warning(LOCATION,
		        "Debris min speed (%f) on %s '%s' is greater than debris max speed (%f), and will be set to debris max "
		        "speed.",
		        sip->debris_min_speed, info_type_name, sip->name, sip->debris_max_speed);
		sip->debris_min_speed = sip->debris_max_speed;
	}
	if (sip->debris_min_rotspeed > sip->debris_max_rotspeed && sip->debris_max_rotspeed >= 0.0f) {
		core::Warning(LOCATION,
		        "Debris min rotation speed (%f) on %s '%s' is greater than debris max rotation speed (%f), and will be "
		        "set to debris max rotation speed.",
		        sip->debris_min_rotspeed, info_type_name, sip->name, sip->debris_max_rotspeed);
	...
	if (sip->debris_min_lifetime > sip->debris_max_lifetime && sip->debris_max_lifetime >= 0.0f) {
		core::Warning(LOCATION,
		        "Debris min lifetime (%f) on %s '%s' is greater than debris max lifetime (%f), and will be set to "
		        "debris max lifetime.",
		        sip->debris_min_lifetime, info_type_name, sip->name, sip->debris_max_lifetime);
	...
	if (sip->debris_min_hitpoints > sip->debris_max_hitpoints && sip->debris_max_hitpoints >= 0.0f) {
		core::Warning(LOCATION,
		        "Debris min hitpoints (%f) on %s '%s' is greater than debris max hitpoints (%f), and will be set to "
		        "debris max hitpoints.",
		        sip->debris_min_hitpoints, info_type_name, sip->name, sip->debris_max_hitpoints);
	...
	sip->warpin_params_index =
	    parse_warp_params(sip->warpin_params_index >= 0 ? &Warp_params[sip->warpin_params_index] : nullptr,
	                      WarpDirection::WARP_IN, info_type_name, sip->name);
	sip->warpout_params_index =
	    parse_warp_params(sip->warpout_params_index >= 0 ? &Warp_params[sip->warpout_params_index] : nullptr,
	                      WarpDirection::WARP_OUT, info_type_name, sip->name);
	...
	if (optional_string("$Vaporize Percent Chance:")) {
		...
		if (sip->vaporize_chance < 0.0f || sip->vaporize_chance > 100.0f) {
			...
			core::Warning(LOCATION, "$Vaporize Percent Chance should be between 0 and 100.0 (read %f). Setting to 0.",
			        sip->vaporize_chance);
		}
		// Percent is nice for modders, but here in the code we want it betwwen 0 and 1.0
	...
	parse_weapon_bank(sip, false, &sip->num_secondary_banks, sip->secondary_bank_weapons,
	                  sip->secondary_bank_ammo_capacity);
	...
	if (optional_string("$Shields:")) {
		...
		if (optional_string("+Spread From LOD:")) {
			...
			if (temp > sip->num_detail_levels)
				core::Warning(LOCATION, "+Spread From LOD for %s was %i whereas ship only has %i detail levels, ignoring...",
	...
	if (optional_string("$Model Point Shield Controls:")) {
		...
		for (auto i = 0; i < num_strings; i++) {
			...
			if (!stricmp(str, "front"))
				...
				core::Warning(LOCATION, "Unrecognized value \"%s\" passed to $Model Point Shield Controls, ignoring...", str);
		}
	}
	...
	if (optional_string("$Hitpoints:")) {
		...
		if (sip->max_hull_strength < 0.0f) {
			core::Warning(LOCATION, "Max hull strength on %s '%s' cannot be less than 0.  Defaulting to 100.\n",
			        info_type_name, sip->name);
	...
	if (optional_string("$Armor Type:")) {
		...
		if (sip->armor_type_idx == -1)
			core::Warning(LOCATION, "Invalid armor name %s specified for hull in %s '%s'", buf, info_type_name, sip->name);
	}
	...
	if (optional_string("$Shield Armor Type:")) {
		...
		if (sip->shield_armor_type_idx == -1)
			core::Warning(LOCATION, "Invalid armor name %s specified for shield in %s '%s'", buf, info_type_name, sip->name);
	}
	...
	if (optional_string("$Flags:")) {
		...
		for (auto i = 0; i < num_strings; i++) {
			...
			for (size_t idx = 0; idx < Num_ship_flags; idx++) {
				if (!stricmp(Ship_flags[idx].name, ship_strings[i])) {
					...
					if (!Ship_flags[idx].in_use)
						core::Warning(LOCATION, "Use of '%s' flag for %s '%s' - this flag is no longer needed.",
			...
			if (!flag_found && (ship_type_index < 0))
				core::Warning(LOCATION, "Bogus string in ship flags: %s\n", ship_strings[i]);
		}
	...
	if (sip->num_primary_banks > MAX_SHIP_PRIMARY_BANKS) {
		core::Error(LOCATION, "%s '%s' has too many primary banks (%d).  Maximum for ships is currently %d.\n",
	...
	if (has_afterburner == 1) {
		...
		if (!(sip->afterburner_fuel_capacity)) {
			core::Warning(LOCATION, "%s '%s' has an afterburner but has no afterburner fuel. Setting fuel to 1",
			        info_type_name, sip->name);
	...
	if (optional_string("$Trails:")) {
		...
		if (trails_warning)
			core::Warning(LOCATION, "%s %s entry has $Trails field specified, but no properties given.", info_type_name,
			        sip->name);
	}
	...
	if (optional_string("$Countermeasure type:")) {
		...
		if (res < 0) {
			core::Warning(LOCATION, "Could not find weapon type '%s' to use as countermeasure on %s '%s'", buf,
			        info_type_name, sip->name);
		} else if (Weapon_info[res].wi_flags[Weapon::Info_Flags::Beam]) {
			core::Warning(LOCATION, "Attempt made to set a beam weapon as a countermeasure on %s '%s'", info_type_name,
			        sip->name);
		} else {
	...
	if (optional_string("$Model Icon Direction:")) {
		...
		if (!stricmp(str, "top")) {
			...
			core::Warning(LOCATION, "Unrecognized value \"%s\" passed to $Model Icon Direction, ignoring...", str);
		}
	...
	if ((sip->bii_index_wing_with_cargo >= 0) && (sip->bii_index_wing < 0 || sip->bii_index_ship_with_cargo < 0))
		core::Warning(LOCATION,
		        "%s '%s' has a wing-with-cargo briefing icon but is missing a wing briefing icon or a ship-with-cargo "
		        "briefing icon!",
		        info_type_name, sip->name);
	if ((sip->bii_index_wing_with_cargo < 0) && (sip->bii_index_wing >= 0) && (sip->bii_index_ship_with_cargo >= 0))
		core::Warning(LOCATION,
		        "%s '%s' has both a wing briefing icon and a ship-with-cargo briefing icon but does not have a "
		        "wing-with-cargo briefing icon!",
		        info_type_name, sip->name);
	...
	while (optional_string("$Thruster Particles:")) {
		...
		if (optional_string("$Thruster Particle Bitmap:"))
			...
			core::Error(LOCATION, "formatting error in the thruster's particle section for %s '%s'\n", info_type_name,
			            sip->name);
	...
	while (optional_string("$Trail:")) {
		...
		if (sip->ct_count >= MAX_SHIP_CONTRAILS) {
			core::Warning(LOCATION, "%s '%s' has more contrails than the max of %d", info_type_name, sip->name,
			        MAX_SHIP_CONTRAILS);
			break;
		}
	...
	while (optional_string("$Thruster:")) {
		...
		if (idx >= 0 && idx < sip->num_maneuvering) {
			...
			if (sip->num_maneuvering < MAX_MAN_THRUSTERS) {
				...
				core::Warning(LOCATION, "Too many maneuvering thrusters on %s '%s'; maximum is %d", info_type_name, sip->name,
				        MAX_MAN_THRUSTERS);
			}
		} else {
			mtp = &manwich;
			core::Warning(LOCATION, "Invalid index (%d) specified for maneuvering thruster on %s '%s'", idx, info_type_name,
			        sip->name);
		}
	...
	if (optional_string("$Glowpoint overrides:")) {
		...
		for (std::vector<std::string>::iterator token = tokens.begin(); token != tokens.end(); ++token) {
			...
			if (seppos == std::string::npos) {
				core::Warning(LOCATION, "Couldn't find ':' seperator in Glowpoint override for ship %s ignoring token",
				        sip->name);
				continue;
			}
			...
			if (gpo == glowpoint_bank_overrides.end()) {
				core::Warning(LOCATION, "Couldn't find preset %s in glowpoints.tbl when parsing ship: %s", name.data(),
				        sip->name);
				continue;
			}
			if (banks == "*") {
	...
	while ((optional_string("$Ship IFF Colors:")) || (optional_string("$Ship IFF Colours:"))) {
		...
		sip->ship_iff_info[iff_data[0]][iff_data[1]] =
		    iff_init_color(iff_color_data[0], iff_color_data[1], iff_color_data[2]);
	}
	...
	if (optional_string("$Target Priority Groups:")) {
		...
		for (auto j = 0; j < num_strings; j++) {
			...
			if (i == num_groups) {
				core::Warning(LOCATION, "Unidentified priority group '%s' set for %s '%s'\n", target_group_strings[j].c_str(),
				        info_type_name, sip->name);
			}
		}
	}
		...
		case 1: {
			...
			if (sp == NULL) {
				if (sip->n_subsystems + n_subsystems >= MAX_MODEL_SUBSYSTEMS) {
					core::Warning(LOCATION,
					        "Number of subsystems for %s '%s' (%d) exceeds max of %d; only the first %d will be used",
					        info_type_name, sip->name, sip->n_subsystems, n_subsystems, MAX_MODEL_SUBSYSTEMS);
					break;
				}
			...
			if (optional_string("$Target Priority:")) {
				...
				for (auto i = 0; i < num_strings; i++) {
					...
					if (j == num_groups) {
						core::Warning(LOCATION, "Unidentified target priority '%s' set for\nsubsystem '%s' in %s '%s'.",
						        tgt_priorities[i].c_str(), sp->subobj_name, info_type_name, sip->name);
					}
				}
			}
			...
			if (optional_string("$ROF:")) {
				...
				if (optional_string("+Use firingpoints")) {
					...
					if (optional_string("+Multiplier:")) {
						...
						core::Warning(LOCATION, "RoF multiplier not set for subsystem\n'%s' in %s '%s'.", sp->subobj_name,
						        info_type_name, sip->name);
					}
				}
			}
			...
			if (optional_string("$Flags:")) {
				...
				if (!errors.empty()) {
					for (auto const& error : errors) {
						core::Warning(LOCATION, "Bogus string in subsystem flags: %s\n", error.c_str());
					}
				}
			...
			if ((sp->flags[Model::Subsystem_Flags::Turret_fixed_fp]) &&
				...
				core::Warning(LOCATION,
				        "\"fixed firingpoints\" flag used without \"use multiple guns\" flag on a subsystem on %s "
			...
			while (optional_string("$animation:")) {
				...
				if (!stricmp(name_tmp, "triggered")) {
					...
					sp->triggers =
					    (queued_animation*)vm_realloc(sp->triggers, sizeof(queued_animation) * (sp->n_triggers + 1));
					core::Verify(sp->triggers != NULL);
					...
					// make sure that the amount of time it takes to accelerate up and down doesn't make it go farther
					// than the angle
	...
	if (hull_percentage_of_hits <= 0.0f) {
		// core::Warning(LOCATION, "The subsystems defined for the %s can take more (or the same) combined damage than the
		// ship itself. Adjust the tables so that the percentages add up to less than 100", sip->name);
	}
...
static void parse_ship_type()
{
	...
	if (optional_string("$Target Priority Groups:")) {
		...
		for (auto j = 0; j < num_strings; j++) {
			...
			if (i == num_groups) {
				core::Warning(LOCATION, "Unidentified priority group '%s' set for objecttype '%s'\n",
...
static void ship_parse_post_cleanup()
{
	...
	for (auto sip = Ship_info.begin(); sip != Ship_info.end(); ++sip) {
		...
		if ((sip->shield_icon_index == 255) && (sip->flags[Ship::Info_Flags::Afterburner]) &&
			...
			core::Warning(LOCATION,
			        "Compatibility warning:\nNo shield icon specified for '%s' but the \"generate icon\" flag is not "
		...
		if (sip->flags[Ship::Info_Flags::Ship_copy]) {
			...
			if (end_string_at_first_hash_symbol(name_tmp)) {
				if (ship_info_lookup(name_tmp) < 0) {
					core::Warning(LOCATION, "Ship %s is a copy, but base ship %s couldn't be found.", sip->name, name_tmp);
		...
		if (!(sip->flags[Ship::Info_Flags::No_collide]) &&
			...
			core::Warning(LOCATION,
			        "$Rotation time: too low; this will disable rotational collisions. All three variables should be "
			        ">= 1.39.\nFix this in ship '%s'\n",
			        sip->name);
		}
		...
		if (sip->min_vel.xyz.x > sip->max_vel.xyz.x || sip->min_vel.xyz.x < 0.0f) {
			error_display(0,
			              "$Player Minimum Velocity X-value (%f) is negative or greater than max velocity X-value "
			              "(%f), setting to zero\nFix for ship '%s'\n",
		...
		if (sip->min_vel.xyz.y > sip->max_vel.xyz.y || sip->min_vel.xyz.y < 0.0f) {
			error_display(0,
			              "$Player Minimum Velocity Y-value (%f) is negative or greater than max velocity Y-value "
			              "(%f), setting to zero\nFix for ship '%s'\n",
		...
		if (sip->min_vel.xyz.z > sip->max_vel.xyz.z || sip->min_vel.xyz.z < 0.0f) {
			error_display(0,
			              "$Player Minimum Velocity Z-value (%f) is negative or greater than max velocity Z-value "
			              "(%f), setting to zero\nFix for ship '%s'\n",
	...
	if (n_tgt_groups > 0) {
		for (size_t i = 0; i < n_tgt_groups; i++) {
			if (!(Ai_tp_list[i].obj_flags.any_set() || Ai_tp_list[i].sif_flags.any_set() ||
			      Ai_tp_list[i].wif_flags.any_set())) {
				// had none of these, check next
				if (Ai_tp_list[i].obj_type == -1) {
					// didn't have this one
					if (!(Ai_tp_list[i].ship_class.size() || Ai_tp_list[i].ship_type.size() ||
					      Ai_tp_list[i].weapon_class.size())) {
						// had nothing - time to issue a warning
						core::Warning(LOCATION, "Target priority group '%s' had no targeting rules issued for it.\n",
						        Ai_tp_list[i].name);
					}
				}
			}
...
void physics_ship_init(object* objp)
{
	...
	// If there's not a value for +Max Glide Speed set in the table, we want this cap to default to the fastest speed
	// the ship can go. However, a negative value means we want no cap, thus allowing nearly infinite maximum gliding
	// speeds.

	// SUSHI: If we are using dynamic glide capping, force the glide cap to 0 (understood by physics.cpp to mean the cap
	// should be dynamic)
...
bool ship::has_display_name() { return !display_name.empty(); }
...
ship_weapon::ship_weapon() { clear(); }

// NOTE: Now that the clear() member function exists, this function only sets the stuff associated with the object and
// ship class.
static void ship_set(int ship_index, int objnum, int ship_type)
{
	...
	if (!subsys_set(objnum)) {
		char err_msg[512];
		sprintf(err_msg,
		        "Unable to allocate ship subsystems, which shouldn't be possible anymore. Current allocation is %d (%d "
		        "in use). No subsystems have been assigned to %s.",
		        Num_ship_subsystems_allocated, Num_ship_subsystems, shipp->ship_name);

		if (Fred_running)
			os::dialogs::Message(os::dialogs::MESSAGEBOX_ERROR, err_msg);
		else
			core::Error(LOCATION, "%s", err_msg);
	}
...
void ship_recalc_subsys_strength(ship* shipp)
{
	...
	for (ship_system = GET_FIRST(&shipp->subsys_list); ship_system != END_OF_LIST(&shipp->subsys_list);
		...
		if (ship_system->current_hits > 0.0f) {
			...
			if ((ship_system->system_info->alive_snd.isValid()) &&
			    !(ship_system->subsys_snd_flags[Ship::Subsys_Sound_Flags::Alive])) {
				obj_snd_assign(shipp->objnum, ship_system->system_info->alive_snd, &ship_system->system_info->pnt, 0,
				               OS_SUBSYS_ALIVE, ship_system);
			...
			if (!(ship_system->subsys_snd_flags[Ship::Subsys_Sound_Flags::Turret_rotation])) {
				if (ship_system->system_info->turret_base_rotation_snd.isValid()) {
					obj_snd_assign(shipp->objnum, ship_system->system_info->turret_base_rotation_snd,
					               &ship_system->system_info->pnt, 0, OS_TURRET_BASE_ROTATION, ship_system);
				...
				if (ship_system->system_info->turret_gun_rotation_snd.isValid()) {
					obj_snd_assign(shipp->objnum, ship_system->system_info->turret_gun_rotation_snd,
					               &ship_system->system_info->pnt, 0, OS_TURRET_GUN_ROTATION, ship_system);
					ship_system->subsys_snd_flags.set(Ship::Subsys_Sound_Flags::Turret_rotation);
				}
			}
			if ((ship_system->flags[Subsystem_Flags::Rotates]) && (ship_system->system_info->rotation_snd.isValid()) &&
			    !(ship_system->subsys_snd_flags[Ship::Subsys_Sound_Flags::Rotate])) {
				obj_snd_assign(shipp->objnum, ship_system->system_info->rotation_snd, &ship_system->system_info->pnt, 0,
				               OS_SUBSYS_ROTATION, ship_system);
			...
			if ((ship_system->system_info->dead_snd.isValid()) &&
			    !(ship_system->subsys_snd_flags[Ship::Subsys_Sound_Flags::Dead])) {
				obj_snd_assign(shipp->objnum, ship_system->system_info->dead_snd, &ship_system->system_info->pnt, 0,
				               OS_SUBSYS_DEAD, ship_system);
				ship_system->subsys_snd_flags.set(Ship::Subsys_Sound_Flags::Dead, false);
			}
		}
	}
	...
	if ((shipp->subsys_info[SUBSYSTEM_ENGINE].type_count > 0) &&
	    (shipp->subsys_info[SUBSYSTEM_ENGINE].aggregate_current_hits <= 0.0f)) {
	...
	for (i = 0; i < sinfo->n_subsystems; i++) {
		...
		if (model_system->model_num < 0) {
			core::Warning(LOCATION,
			        ...
			        "ship!\n\n(This warning means that a subsystem was present in the table entry and not present in "
		...
		ship_system->system_info =
		    model_system; // set the system_info pointer to point to the data read in from the model
		...
		if ((The_mission.ai_profile->flags[AI::Profile_Flags::Advanced_turret_fov_edge_checks]) ||
		    (model_system->flags[Model::Subsystem_Flags::Fov_edge_check]))
			ship_system->flags.set(Ship::Subsystem_Flags::FOV_edge_check);
		if ((The_mission.ai_profile->flags[AI::Profile_Flags::Require_turret_to_have_target_in_fov]) ||
		    (model_system->flags[Model::Subsystem_Flags::Fov_required]))
		...
		ship_system->max_hits =
		    model_system->max_subsys_strength; // * shipp->ship_max_hull_strength / sinfo->max_hull_strength;
		...
		ship_system->turret_next_fire_stamp =
		    timestamp((int)frand_range(1.0f, 500.0f)); // next time this turret can fire
		...
		for (k = 0; k < MAX_SHIP_PRIMARY_BANKS; k++) {
			if (model_system->primary_banks[k] != -1) {
				...
				ship_system->weapons.primary_bank_capacity[j] =
				    model_system->primary_bank_capacity[k]; // added by Goober5000
		...
		if ((ship_system->system_info->flags[Model::Subsystem_Flags::Turret_salvo]) &&
		    (ship_system->system_info->flags[Model::Subsystem_Flags::Turret_fixed_fp])) {
			core::Warning(LOCATION,
		...
		if ((ship_system->system_info->flags[Model::Subsystem_Flags::Turret_salvo]) &&
		    (ship_system->system_info->flags[Model::Subsystem_Flags::Use_multiple_guns])) {
			core::Warning(LOCATION,
		...
		if ((ship_system->system_info->flags[Model::Subsystem_Flags::Share_fire_direction]) &&
		    (!(ship_system->system_info->flags[Model::Subsystem_Flags::Turret_salvo]) &&
		     !(ship_system->system_info->flags[Model::Subsystem_Flags::Use_multiple_guns]))) {
			core::Warning(LOCATION,
			        "\"share fire direction\" flag used with turret which does not have the \"salvo mode\" or \"use "
			        "multiple guns\" flag set\nsubsystem '%s' on ship type '%s'.\n\"share fire direction\" flag is "
			        "ignored\n",
			        model_system->subobj_name, sinfo->name);
		...
		for (k = 0; k < ship_system->weapons.num_primary_banks; k++) {
			...
			if (weapon_size > 0.0f) { // Non-ballistic primaries are supposed to have a cargo_size of 0
				ship_system->weapons.primary_bank_ammo[k] =
				    (Fred_running ? 100
				                  : (int)std::lround(ship_system->weapons.primary_bank_capacity[k] / weapon_size));
			}
		}
...
static void ship_find_warping_ship_helper(object* objp, dock_function_info* infop)
{
	...
	if (Ships[objp->instance].is_arriving(ship::warpstage::BOTH, true) ||
	    Ships[objp->instance].flags[Ship_Flags::Depart_warp]) {
#ifndef NDEBUG
		...
		if (infop->maintained_variables.bool_value) {
			...
			// core::Warning(LOCATION, "Ship %s and its docked ship %s are arriving or departing at the same time.\n",
...
void ship_render_show_ship_cockpit(object* objp)
{
	...
	model_render_immediate(&render_info, Ship_info[Ships[objp->instance].ship_info_index].model_num, &objp->orient,
	                       &vmd_zero_vector); // Render ship model with fixed detail level 0 so its not switching LOD
	                                          // when moving away from origin
...
static void ship_add_cockpit_display(cockpit_display_info* display, int cockpit_model_num)
{
	...
	if (display->bg_filename[0] != '\0') {
		...
		if (new_display.background < 0) {
			core::Warning(LOCATION, "Unable to load background %s for cockpit display %s", display->bg_filename,
			        display->name);
		}
	}
	...
	if (display->fg_filename[0] != '\0') {
		...
		if (new_display.foreground < 0) {
			core::Warning(LOCATION, "Unable to load background %s for cockpit display %s", display->fg_filename,
			        display->name);
		}
	}
...
void ship_wing_cleanup(int shipnum, wing* wingp)
{
	...
	if (wingp->current_count == 0) {
		...
		if ((wingp->current_wave == wingp->num_waves) &&
		    (wingp->total_destroyed + wingp->total_departed + wingp->total_vanished == wingp->total_arrived_count)) {
			...
			if ((wingp->total_destroyed + wingp->total_departed + wingp->total_vanished) !=
				...
				for (ship_obj* so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
					...
					if ((Ships[Objects[so->objnum].instance].wingnum == WING_INDEX(wingp)) &&
					    !(Ships[Objects[so->objnum].instance].is_dying_or_departing())) {
						// TODO: I think this Int3() is triggered when a wing whose ships are all docked to ships of
						// another wing departs.  It can be reliably seen in TVWP chapter 1 mission 7, when Torino and
						// Iota wing depart. Not sure how to fix this. -- Goober5000
						Int3();
					}
				}
			}
...
void ship_cleanup(int shipnum, int cleanup_mode)
{
	...
	if (cleanup_mode == SHIP_DESTROYED) {
		...
		if (!(shipp->flags[Ship_Flags::Ignore_count]) ||
		...
		// let the event music system know an enemy was destroyed (important for deciding when to transition from battle
		// to normal music)
	...
	if ((cleanup_mode == SHIP_DEPARTED_WARP) || (cleanup_mode == SHIP_DEPARTED_BAY) ||
	    (cleanup_mode == SHIP_DEPARTED)) { // see if this ship departed within the radius of a jump node -- if so, put
		                                   // the node name into
 ...
 */
int ship_explode_area_calc_damage(vec3d* pos1, vec3d* pos2, float inner_rad, float outer_rad, float max_damage,
                                  float max_blast, float* damage, float* blast)
{
	float dist;
...
static void ship_blow_up_area_apply_blast(object* exp_objp)
{
	...
	if (shockwave_speed > 0) {
		...
		for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
			...
			if (ship_explode_area_calc_damage(&exp_objp->pos, &objp->pos, inner_rad, outer_rad, max_damage, max_blast,
			                                  &damage, &blast) == -1) {
				continue;
			}
...
static void do_dying_undock_physics(object* dying_objp, ship* dying_shipp)
{
	...
	while (object_is_dead_docked(dying_objp)) {
		...
		physics_apply_whack(&impulse_vec, &pos, &docked_objp->phys_info, &docked_objp->orient,
		                    docked_objp->phys_info.mass);
		...
		physics_apply_whack(&impulse_vec, &pos, &dying_objp->phys_info, &dying_objp->orient,
		                    dying_objp->phys_info.mass);
	...
	if (shipp->flags[Ship_Flags::Dying]) {
		...
		if (shipp->flags[Ship_Flags::Vaporize]) {
			if (timestamp_elapsed(shipp->final_death_time)) {
				// play death sound
				snd_play_3d(gamesnd_get_game_sound(GameSounds::VAPORIZED), &objp->pos, &View_position, objp->radius,
				            NULL, 0, 1.0f, SND_PRIORITY_MUST_PLAY);
		...
		if (!(Ai_info[shipp->ai_index].ai_flags[AI::AI_Flags::Kamikaze]) && ship_get_exp_propagates(shipp) &&
		    (sip->death_roll_r_mult > 0.0f)) {
			if (timestamp_elapsed(shipp->next_fireball)) {
				...
				fireball_create(&outpnt, fireball_type, FIREBALL_LARGE_EXPLOSION, OBJ_INDEX(objp), rad, false,
				                &objp->phys_info.vel);
		...
		if (knossos_ship) {
			if (timestamp_elapsed(shipp->next_fireball)) {
				...
				pe.normal_variance =
				    pef.variance; //	How close they stick to that normal 0=on normal, 1=180, 2=360 degree
		...
		if ((time_until_minor_explosions < 500) && (time_until_minor_explosions > 100) &&
		    (!shipp->pre_death_explosion_happened)) {
			...
			for (int zz = 0; zz < zz_max; zz++) {
				...
				fireball_create(&outpnt, fireball_type, FIREBALL_MEDIUM_EXPLOSION, OBJ_INDEX(objp), rad, false,
				                &objp->phys_info.vel);
			}
		}
		...
		if (timestamp_elapsed(shipp->final_death_time)) {
			...
			snd_play_3d(gamesnd_get_game_sound(sound_index), &objp->pos, &View_position, objp->radius, NULL, 0, 1.0f,
			            SND_PRIORITY_MUST_PLAY);
			if (objp == Player_obj)
				joy_ff_explode();
			...
			if (ship_get_exp_propagates(shipp)) {
				...
				fireball_objnum = fireball_create(&objp->pos, fireball_type, FIREBALL_LARGE_EXPLOSION, OBJ_INDEX(objp),
				                                  big_rad, false, &objp->phys_info.vel);
				...
				shipp->end_death_time = shipp->really_final_death_time = timestamp(
				    fl2i(explosion_life * 1000.0f) / 5); // Wait till 30% of vclip time before breaking the ship up.
			}
		...
		if (timestamp_elapsed(shipp->really_final_death_time)) {
			...
			if (shipp->large_ship_blowup_index >= 0) {
				if (shipfx_large_blowup_do_frame(shipp, flFrametime)) {
					...
					ship_cleanup(ship_num,
					             SHIP_DESTROYED); // call ship function to clean up after the ship is destroyed.
				}
				return;
			}
...
static void ship_check_player_distance_sub(player* p, int multi_target = -1)
{
	...
	if (give_warning_to_player) {
		...
		if (p->distance_warning_count <= PLAYER_DISTANCE_MAX_WARNINGS) {
			message_send_builtin_to_player(MESSAGE_STRAY_WARNING, NULL, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_SOON, 0, 0,
			                               multi_target, -1);
		}
	...
	if (!(p->flags & PLAYER_FLAGS_FORCE_MISSION_OVER) &&
	    ((p->distance_warning_count > PLAYER_DISTANCE_MAX_WARNINGS) || (dist > PLAYER_MAX_DIST_END))) {
		//		DKA 5/17/99 - DON'T force warpout.  Won't work multiplayer.  Blow up ship.
		if (!(p->flags & PLAYER_FLAGS_DIST_TO_BE_KILLED)) {
			message_send_builtin_to_player(MESSAGE_STRAY_WARNING_FINAL, NULL, MESSAGE_PRIORITY_HIGH,
			                               MESSAGE_TIME_IMMEDIATE, 0, 0, multi_target, -1);
	...
	if ((dist < PLAYER_MAX_DIST_WARNING) && (p->flags & PLAYER_FLAGS_DIST_WARNING) &&
	    !(p->flags & PLAYER_FLAGS_DIST_TO_BE_KILLED)) {
		p->flags &= ~PLAYER_FLAGS_DIST_WARNING;
		...
		if (MULTIPLAYER_MASTER) {
			...
			for (idx = 0; idx < MAX_PLAYERS; idx++) {
				if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
				    ...
				    (Objects[Net_players[idx].m_player->objnum].type != OBJ_GHOST)) {
					// if bad, blow him up
...
void ship_process_post(object* obj, float frametime)
{
	...
	if (!(shipp->flags[Ship_Flags::Ammo_count_recorded])) {
		...
		for (int i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++) {
			if (red_alert_mission()) {
				max_missiles =
				    get_max_ammo_count_for_bank(shipp->ship_info_index, i, shipp->weapons.secondary_bank_weapons[i]);
		...
		for (int i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) {
			if (red_alert_mission()) {
				max_missiles = get_max_ammo_count_for_primary_bank(shipp->ship_info_index, i,
				                                                   shipp->weapons.primary_bank_weapons[i]);
	...
	if ((!(shipp->is_arriving()) || (Ai_info[shipp->ai_index].mode == AIM_BAY_EMERGE) ||
	     ((Warp_params[shipp->warpin_params_index].warp_type == WT_IN_PLACE_ANIM) &&
		...
		if ((obj->flags[Object::Object_Flags::Player_ship]) && !Player_use_ai) {
			...
			if (aip->ai_flags[AI::AI_Flags::Being_repaired, AI::AI_Flags::Awaiting_repair]) {
				if (aip->support_ship_objnum >= 0) {
					if (vm_vec_dist_quick(&obj->pos, &Objects[aip->support_ship_objnum].pos) <
					    (obj->radius + Objects[aip->support_ship_objnum].radius) * 1.25f)
						return;
				}
			}
...
static void ship_set_default_weapons(ship* shipp, ship_info* sip)
{
	...
	if (pm->n_guns > sip->num_primary_banks) {
		...
		core::Warning(LOCATION, "There are %d primary banks specified for %s\nbut only %d primary banks in the model\n",
...
static void ship_init_afterburners(ship* shipp)
{
	...
	for (int i = 0; i < pm->n_thrusters; i++) {
		...
		for (int j = 0; j < bank->num_points; j++) {
			...
			ci->w_start =
			    bank->points[j].radius * sip->afterburner_trail_width_factor; // width * table loaded width factor
...
int ship_create(matrix* orient, vec3d* pos, int ship_type, const char* ship_name)
{
	...
	if (Fred_running) {
		...
		if (t >= SHIPS_LIMIT) {
			core::Error(LOCATION,
			      XSTR("There is a limit of %d ships in the mission at once.  Please be sure that you do not have more "
			           "than %d ships present in the mission at the same time.",
			           1495),
	...
	if (strlen(sip->pof_file_hud)) {
		// check to see if a "real" ship uses this model. if so, load it up for him so that subsystems are setup
		// properly
	...
	ship_set_default_weapons(shipp,
	...
	ai_clear_ship_goals(&Ai_info[Ships[n].ai_index]); // only do this one here.  Can't do it in init_ai because it might
	                                                  // wipe out goals in mission file
				...
				for (k = 0; k < pm->paths[i].verts[j].nturrets; k++) {
					...
					if (ss == END_OF_LIST(&Ships[n].subsys_list))
						core::Warning(LOCATION,
						        ...
						        "%d\nTurret model id:%d\n\nThis probably means that the turret was not specified in "
						        "the ship table(s).",
...
void change_ship_type(int n, int ship_type, int by_sexp)
{
		...
		for (i = 0; i < MAX_AI_INFO; i++) {
			if (Ai_info[i].shipnum > -1) {
				...
				for (int j = 0; j < MAX_AI_GOALS; j++) {
					// POSSIBLE OPTIMIZATION: goals[0] should be the active goal, so we might be able to reuse
					// target_subsys_parent for that instead of doing ship_name_lookup()
					if (goals[j].ai_mode == AI_GOAL_DESTROY_SUBSYSTEM &&
					    !(goals[j].flags[AI::Goal_Flags::Subsys_needs_fixup])) { // If the subsystem name hasn't been
						                                                         // parsed yet, we're fine.
	...
	while (ss != END_OF_LIST(&sp->subsys_list)) {
		if (num_saved_subsystems == sip_orig->n_subsystems) {
			core::Error(LOCATION, "Subsystem mismatch while changing ship class from '%s' to '%s'!", sip_orig->name,
			      sip->name);
			break;
		}
			...
			// subsystem, add them to the subsystem_matches vector and remove them from our "to be checked" list.
			{
				...
				while (it != target_matches.end()) {
					...
					if (aip->targeted_subsys == ss) {
						ivec3 temp = {
						    *it, -1,
						    num_saved_subsystems}; // -1 for the "goal" index means targeted, not actually a goal
						subsystem_matches.push_back(temp);
						aip->targeted_subsys = NULL; // Clear this so that aip->last_subsys_target won't point to a
						                             // subsystem from the old list later
	...
	if (!Fred_running) {
		if ((p_objp->flags[Mission::Parse_Object_Flags::OF_Force_shields_on]) &&
		    (sp->ship_max_shield_strength > 0.0f)) {
			objp->flags.remove(Object::Object_Flags::No_shields);
		} else if ((p_objp->flags[Mission::Parse_Object_Flags::OF_No_shields]) ||
		           (sp->ship_max_shield_strength == 0.0f)) {
			objp->flags.set(Object::Object_Flags::No_shields);
			// Since there's not a mission flag set to be adjusting this, see if there was a change from a ship that
			// normally has shields to one that doesn't, and vice versa
		} else if (!(sip_orig->flags[Info_Flags::Intrinsic_no_shields]) &&
		           (sip->flags[Info_Flags::Intrinsic_no_shields])) {
			objp->flags.set(Object::Object_Flags::No_shields);
		} else if ((sip_orig->flags[Info_Flags::Intrinsic_no_shields]) &&
		           !(sip->flags[Info_Flags::Intrinsic_no_shields]) && (sp->ship_max_shield_strength > 0.0f)) {
			objp->flags.remove(Object::Object_Flags::No_shields);
		}
	}
	...
	sp->afterburner_fuel =
	    MAX(0, sip->afterburner_fuel_capacity - (sip_orig->afterburner_fuel_capacity - sp->afterburner_fuel));
	...
	if (sip->flags[Ship::Info_Flags::Afterburner]) {
		...
		for (int h = 0; h < pm->n_thrusters; h++) {
			for (int j = 0; j < pm->thrusters[h].num_points; j++) {
				...
				ci->w_start = pm->thrusters[h].points[j].radius *
				              sip->afterburner_trail_width_factor; // width * table loaded width factor
				...
				ci->n_fade_out_sections =
				    sip->afterburner_trail_faded_out_sections; // table loaded n sections to be faded out
	...
	else if (sip_orig->flags[Ship::Info_Flags::Ship_class_dont_collide_invis]) // changing FROM a
	                                                                           // don't-collide-invisible ship class
...
static int ship_stop_fire_primary_bank(object* obj, int bank_to_stop)
{
	...
	if (shipp->primary_rotate_rate[bank_to_stop] > 0.0f)
		shipp->primary_rotate_rate[bank_to_stop] -=
		    Weapon_info[swp->primary_bank_weapons[bank_to_stop]].weapon_submodel_rotate_accell * flFrametime;
	if (shipp->primary_rotate_rate[bank_to_stop] < 0.0f)
...
bool in_autoaim_fov(ship* shipp, int bank_to_fire, object* obj)
{
	...
	has_converging_autoaim =
	    ((sip->aiming_flags[Ship::Aiming_Flags::Autoaim_convergence] ||
	      (The_mission.ai_profile->player_autoaim_fov[Game_skill_level] > 0.0f && !(Game_mode & GM_MULTIPLAYER))) &&
	     aip->target_objnum != -1);
	has_autoaim =
	    ((has_converging_autoaim || (sip->aiming_flags[Ship::Aiming_Flags::Autoaim])) && aip->target_objnum != -1);
...
int ship_fire_primary(object* obj, int stream_weapons, int force)
{
	...
	if (num_primary_banks == 1)
		for (i = 0; i < swp->num_primary_banks; i++) {
			if (i != swp->current_primary_bank)
				ship_stop_fire_primary_bank(obj, i);
		}
	...
	has_autoaim =
	    ((has_converging_autoaim || (sip->aiming_flags[Ship::Aiming_Flags::Autoaim])) && aip->target_objnum != -1);
	needs_target_pos =
	    ((has_autoaim || (sip->aiming_flags[Ship::Aiming_Flags::Auto_convergence])) && aip->target_objnum != -1);
	...
	for (i = 0; i < num_primary_banks; i++) {
		...
		// if this is a targeting laser, start it up   ///- only targeting laser if it is tag-c, otherwise it's a
		// fighter beam -Bobboau
		if ((winfo_p->wi_flags[Weapon::Info_Flags::Beam]) && (winfo_p->tag_level == 3) &&
		    (shipp->flags[Ship_Flags::Trigger_down]) && (winfo_p->b_info.beam_type == BEAM_TYPE_C)) {
		...
		if (winfo_p->burst_shots > swp->burst_counter[bank_to_fire]) {
			...
			next_fire_delay =
			    (((float)rand()) /
			     (((float)RAND_MAX + 1.0f) * (winfo_p->max_delay - winfo_p->min_delay + 1.0f) + winfo_p->min_delay)) *
			    1000.0f;
		} else {
		...
		if ((num_primary_banks > 1) && !(winfo_p->wi_flags[Weapon::Info_Flags::No_linked_penalty]) &&
		    !(The_mission.ai_profile->flags[AI::Profile_Flags::Disable_linked_fire_penalty])) {
		...
		if (winfo_p->fof_spread_rate > 0.0f) {
			...
			float reset_amount =
			    (timestamp_until(swp->last_primary_fire_stamp[bank_to_fire]) / 1000.0f) * winfo_p->fof_reset_rate;
		...
		if (pm->n_guns > 0) {
			...
			if (needs_target_pos) {
				...
				polish_predicted_target_pos(winfo_p, &Objects[aip->target_objnum], &target_position,
				                            &predicted_target_pos, dist_to_target, &last_delta_vec, 1);
			...
			if (winfo_p
			        ->wi_flags[Weapon::Info_Flags::Beam]) { // the big change I made for fighter beams, if there beams
				                                            // fill out the Fire_Info for a targeting laser then fire
				                                            // it, for each point in the weapon bank -Bobboau
				float t;
				if (winfo_p->burst_shots > swp->burst_counter[bank_to_fire]) {
					t = winfo_p->burst_delay;
					swp->burst_counter[bank_to_fire]++;
				} else {
					t = winfo_p->fire_wait; // doing that time scale thing on enemy fighter is just ugly with beams,
					                        // especaly ones that have careful timeing
					swp->burst_counter[bank_to_fire] = 0;
				}
			...
			} else // if this isn't a fighter beam, do it normally -Bobboau
			{
				...
				// otherwise shots is the number of times all points will be fired (used mostly for the 'shotgun'
				// effect)
				if (sip->flags[Ship::Info_Flags::Dyn_primary_linking]) {
					numtimes = 1;
				...
				for (w = 0; w < numtimes; w++) {
					...
					if (weapon_model)
						if ((weapon_model->n_guns <= swp->external_model_fp_counter[bank_to_fire]) ||
						    (swp->external_model_fp_counter[bank_to_fire] < 0))
							swp->external_model_fp_counter[bank_to_fire] = 0;

					for (j = 0; j < points; j++) {
						int pt; // point
						if ((winfo_p->wi_flags[Weapon::Info_Flags::Cycle]) ||
						    (sip->flags[Ship::Info_Flags::Dyn_primary_linking])) {
						...
						int sub_shots = 1;
						// Use 0 instead of bank_to_fire as index when checking the number of external weapon model
						// firingpoints
						...
						for (int s = 0; s < sub_shots; s++) {
							...
							if (weapon_model && weapon_model->n_guns) {
								if (winfo_p->wi_flags[Weapon::Info_Flags::External_weapon_fp]) {
									vm_vec_add2(
									    &pnt,
									    &weapon_model->gun_banks[0].pnt[swp->external_model_fp_counter[bank_to_fire]]);
								} else {
							...
							*/
							if (has_autoaim && in_automatic_aim_fov) {
								...
								if ((sip->aiming_flags[Ship::Aiming_Flags::Auto_convergence]) &&
								    (aip->target_objnum != -1)) {
									// auto convergence
							...
							if (winfo_p->wi_flags[Weapon::Info_Flags::Apply_Recoil]) { // Function to add recoil
								                                                       // functionality - DahBlount
								vec3d local_impulse = firing_orient.vec.fvec;

								float recoil_force = (winfo_p->mass * winfo_p->max_speed * winfo_p->recoil_modifier *
								                      sip->ship_recoil_modifier);
							...
							// of weapon_create
							weapon_objnum =
							    weapon_create(&firing_pos, &firing_orient, weapon_idx, OBJ_INDEX(obj), new_group_id, 0,
							                  0, swp->primary_bank_fof_cooldown[bank_to_fire]);
							...
							weapon_set_tracking_info(weapon_objnum, OBJ_INDEX(obj), aip->target_objnum,
							...
							if (winfo_p->wi_flags[Weapon::Info_Flags::Flak]) {
								...
								if (target != NULL) {
									set_predicted_enemy_pos(&predicted_pos, obj, &target->pos, &target->phys_info.vel,
									                        aip);
								...
								if ((target != NULL) && (range_to_target < flak_range)) {
									...
									flak_set_range(&Objects[weapon_objnum],
									               flak_range - winfo_p->untargeted_flak_range_penalty);
								}

								if ((winfo_p->muzzle_flash >= 0) &&
								    (((shipp == Player_ship) && (vm_vec_mag(&Player_obj->phys_info.vel) >= 45)) ||
								     (shipp != Player_ship))) {
									flak_muzzle_flash(&firing_pos, &obj->orient.vec.fvec, &obj->phys_info,
									                  swp->primary_bank_weapons[bank_to_fire]);
								}
							}
							// create the muzzle flash effect
							if ((obj != Player_obj) || (sip->flags[Ship::Info_Flags::Show_ship_model]) ||
							    (Viewer_mode)) {
							...
							if ((winfo_p->wi_flags[Weapon::Info_Flags::Shudder]) && (obj == Player_obj) &&
							...
							num_fired++;
							shipp->last_fired_point[bank_to_fire] =
							    (shipp->last_fired_point[bank_to_fire] + 1) % num_slots;
						}
					}
			...
			if (sound_played != winfo_p->launch_snd) {
				...
				if (obj == Player_obj) {
					if (winfo_p->launch_snd.isValid()) {
						...
						// Update the last timestamp until continous fire is over, so we have the timestamp of the
						// cease-fire.
						...
						if ((winfo_p->pre_launch_snd.isValid()) // If this weapon type has a pre-fire sound
						    && ((timestamp() - swp->last_primary_fire_sound_stamp[bank_to_fire]) >=
						        winfo_p->pre_launch_snd_min_interval) // and if we're past our minimum delay from the
						                                              // last cease-fire
						    && (shipp->was_firing_last_frame[bank_to_fire] ==
						        0) // and if we are at the beginning of a firing stream
						) {
							snd_play(gamesnd_get_game_sound(winfo_p->pre_launch_snd), 0.0f, 1.0f,
							         SND_PRIORITY_MUST_PLAY); // play it
						} else {                              // Otherwise, play normal firing sounds
							// HACK
							if (winfo_p->launch_snd == gamesnd_id(GameSounds::AUTOCANNON_SHOT)) {
								snd_play(gamesnd_get_game_sound(winfo_p->launch_snd), 0.0f, 1.0f,
						...
						if (sw_pl->current_primary_bank >= 0) {
							...
							int force_level = (int)((wip->armor_factor + wip->shield_factor * 0.2f) *
							                            (wip->damage * wip->damage - 7.5f) * 0.45f +
							                        0.6f) *
							                      10 +
							                  2000;
...
static void ship_start_targeting_laser(ship* shipp)
{
	...
	if ((shipp->weapons.primary_bank_weapons[0] >= 0) &&
	    (Weapon_info[shipp->weapons.primary_bank_weapons[0]].wi_flags[Weapon::Info_Flags::Beam]) &&
	    (Weapon_info[shipp->weapons.primary_bank_weapons[0]].b_info.beam_type == BEAM_TYPE_C)) {
		bank0_laser = 1;
	}
	if ((shipp->weapons.primary_bank_weapons[1] >= 0) &&
	    (Weapon_info[shipp->weapons.primary_bank_weapons[1]].wi_flags[Weapon::Info_Flags::Beam]) &&
	    (Weapon_info[shipp->weapons.primary_bank_weapons[1]].b_info.beam_type == BEAM_TYPE_C)) {
		bank1_laser = 1;
	}
				...
				for (mo = GET_FIRST(&Missile_obj_list); mo != END_OF_LIST(&Missile_obj_list); mo = GET_NEXT(mo)) {
					...
					if ((mobjp != first_objp) && (mobjp->parent_sig == obj->parent_sig)) {
						if (Weapon_info[Weapons[mobjp->instance].weapon_info_index]
						        .wi_flags[Weapon::Info_Flags::Remote]) {
...
//	to fire the missiles when allow_swarm is NOT set.  They don't actually get fired on a call here unless allow_swarm
//is set. 	When you want to fire swarmers, you call this function with allow_swarm NOT set and frame interval 	code comes
//aruond and fires it.
...
int ship_fire_secondary(object* obj, int allow_swarm)
{
	...
	if (wip->wi_flags[Weapon::Info_Flags::Tagged_only]) {
		if (!ship_is_tagged(&Objects[aip->target_objnum])) {
			if (obj == Player_obj) {
				if (!Weapon_energy_cheat) {
					HUD_sourced_printf(HUD_SOURCE_HIDDEN, NOX("Cannot fire %s if target is not tagged"),
					                   wip->get_display_string());
					snd_play(gamesnd_get_game_sound(ship_get_sound(Player_obj, GameSounds::OUT_OF_MISSLES)));
					swp->next_secondary_fire_stamp[bank] = timestamp(800); // to avoid repeating messages
					return 0;
				}
		...
		if (ship_weapon_maybe_fail(shipp)) {
			if (obj == Player_obj)
				if (ship_maybe_play_secondary_fail_sound(wip)) {
					...
					end_string_at_first_hash_symbol(missile_name);
					HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("Cannot fire %s due to weapons system damage", 489),
					                   missile_name);
				}
			goto done_secondary;
		}
	...
	if (pm->n_missiles > 0) {
		...
		if (check_ammo && (swp->secondary_bank_ammo[bank] <= 0)) {
			if (shipp->objnum == OBJ_INDEX(Player_obj)) {
				if (ship_maybe_play_secondary_fail_sound(wip)) {
					//					HUD_sourced_printf(HUD_SOURCE_HIDDEN, "No %s missiles left in bank",
		...
		for (j = start_slot; j <= end_slot; j++) {
			...
			if (weapon_model && weapon_model->n_guns) {
				...
				if (wip->wi_flags[Weapon::Info_Flags::External_weapon_fp]) {
					if ((weapon_model->n_guns <= swp->external_model_fp_counter[external_bank]) ||
			...
			// of weapon_create
			weapon_num = weapon_create(&firing_pos, &firing_orient, weapon_idx, OBJ_INDEX(obj), -1,
			                           aip->current_target_is_locked);
			...
			if (weapon_num >= 0) {
				...
				weapon_set_tracking_info(weapon_num, OBJ_INDEX(obj), aip->target_objnum, aip->current_target_is_locked,
				                         aip->targeted_subsys);
				has_fired = true;
				...
				if ((wip->wi_flags[Weapon::Info_Flags::Shudder]) && (obj == Player_obj) &&
				    !(Game_mode & GM_STANDALONE_SERVER)) {
					// calculate some arbitrary value between 100
					// (mass * velocity) / 10
...
done_secondary:
	...
	// this bug was made by AL, when he assumed he had to take the next fire_wait time remaining and add 250 ms of delay
	// to it, and put it in the next valid bank. for the player to have a 250 ms of penalty
	//
	// what that caused was that the next valid bank inherited the current valid banks FULL fire delay. since he used
	// the Weapon_info struct that has
	// no information / member that stores the next valids banks remaning fire_wait delay.
	//
	// what he should have done was to check of the next valid bank had any fire delay that had elapsed, if it had
	// elapsed, then it would have no firedelay. and then add 250 ms of delay. in effect, this way there is no penalty if
	// there is any firedelay remaning in the next valid bank. the delay is there to prevent things like Trible/Quad Fire
	// Trebuchets.
	//
	// niffiwan: only try to switch banks if object has multiple banks, and firing bank is the current bank
	if ((obj->flags[Object::Object_Flags::Player_ship]) && (swp->secondary_bank_ammo[bank] <= 0) &&
	    (swp->num_secondary_banks >= 2) && (bank == swp->current_secondary_bank)) {
		if (ship_select_next_secondary(
		        obj)) { // DTP here we switch to the next valid bank, but we can't call weapon_info on next fire_wait

			if (timestamp_elapsed(shipp->weapons.next_secondary_fire_stamp
			                          [shipp->weapons.current_secondary_bank])) { // DTP, this is simply a copy of the
				                                                                  // manual cycle functions
				shipp->weapons.next_secondary_fire_stamp[shipp->weapons.current_secondary_bank] =
				    timestamp(1000); // Bumped from 250 to 1000 because some people seem to be to triggerhappy :).
...
int ship_select_next_primary(object* objp, int direction)
{
	...
	if (swp->num_primary_banks == 0) {
		...
		if (objp == Player_obj) {
			HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("This ship has only one primary weapon: %s", 491),
			                   Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]].get_display_string(),
			                   swp->current_primary_bank + 1);
			gamesnd_play_error_beep();
		}
		return 0;
	...
	if (ship_has_a_ballistic_primary(shipp)) {
		...
		if (shipp->flags[Ship_Flags::Primary_linked]) {
			for (i = 0; i < swp->num_primary_banks; i++) {
				if (!Weapon_info[swp->primary_bank_weapons[i]].wi_flags[Weapon::Info_Flags::Nolink] &&
				    primary_out_of_ammo(swp, i)) // We only care if linkable primaries are out of ammo.
				{
					shipp->flags.remove(Ship_Flags::Primary_linked);
	...
	if ((swp->current_primary_bank != original_bank) ||
	    ((shipp->flags[Ship_Flags::Primary_linked]) != original_link_flag)) {
		if (objp == Player_obj) {
...
int ship_select_next_secondary(object* objp)
{
	...
	if (swp->num_secondary_banks == 0) {
		...
		if (objp == Player_obj) {
			HUD_sourced_printf(
			    HUD_SOURCE_HIDDEN, XSTR("This ship has only one secondary weapon: %s", 493),
			    Weapon_info[swp->secondary_bank_weapons[swp->current_secondary_bank]].get_display_string(),
			    swp->current_secondary_bank + 1);
			gamesnd_play_error_beep();
		}
		return 0;
...
int ship_info_lookup(const char* token)
{
	...
	if (*p == '#') {
		if (strlen(token) > NAME_LENGTH - 3) {
			// If the below sprintf would exceed NAME_LENGTH (taking \0 terminator into account), give a warning and
			// return.
			core::Warning(LOCATION, "Token [%s] is too long to be parenthesized by ship_info_lookup()!\n", token);
			return -1;
		}
	...
	// oops
		...
		core::Warning(LOCATION, "Unrecognized hash symbol.  Contact a programmer!");
		return -1;
	}
...
int get_subsystem_pos(vec3d* pos, object* objp, ship_subsys* subsysp)
{
	...
	if (mss->subobj_num == -1) {
		...
		find_submodel_instance_world_point(pos, Ships[objp->instance].model_instance_num, mss->subobj_num,
		                                   &objp->orient, &objp->pos);
	}
...
void ship_get_eye(vec3d* eye_pos, matrix* eye_orient, object* obj, bool do_slew, bool from_origin)
{
	...
	if (ep->parent >= 0 && pm->submodel[ep->parent].can_move) {
		find_submodel_instance_point_orient(eye_pos, eye_orient, shipp->model_instance_num, ep->parent, &ep->pnt,
		                                    &vmd_identity_matrix);
		vec3d tvec = *eye_pos;
		...
		model_find_world_point(eye_pos, &ep->pnt, pm->id, ep->parent, &obj->orient,
		                       from_origin ? &vmd_zero_vector : &obj->pos);
		*eye_orient = obj->orient;
	}
...
ship_subsys* ship_get_indexed_subsys(ship* sp, int index, vec3d* attacker_pos)
{
	...
	if (index < 0) {
		...
		subsys_type = -index;
		if (sp->subsys_info[subsys_type].aggregate_current_hits <=
		    0.0f) // if there are no hits, no subsystem to attack.
			return NULL;
		...
		core::Warning(LOCATION, "Unable to get a nonspecific subsystem of index %d on ship %s!", index, sp->ship_name);
		return NULL;
	}
	...
	core::Warning(LOCATION,
	        "In ship_get_indexed_subsys, unable to get a subsystem of index %d on ship %s, due to a broken subsystem "
	        "reference!  This is most likely due to a table/model mismatch.",
	        index, sp->ship_name);
	return NULL;
}
...
float ship_calculate_rearm_duration(object* objp)
{
	...
	if ((The_mission.flags[Mission::Mission_Flags::Support_repairs_hull]) && (max_hull_repair > objp->hull_strength) &&
	    (sip->sup_hull_repair_rate > 0.0f)) {
		hull_rep_time =
		    (max_hull_repair - objp->hull_strength) / (sp->ship_max_hull_strength * sip->sup_hull_repair_rate);
	}
	...
	for (i = 0; i < swp->num_primary_banks; i++) {
		...
		if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
			...
			num_reloads =
			    (swp->primary_bank_start_ammo[i] - swp->primary_bank_ammo[i]) / REARM_NUM_BALLISTIC_PRIMARIES_PER_BATCH;

			// take into account a fractional reload
			if ((swp->primary_bank_start_ammo[i] - swp->primary_bank_ammo[i]) %
			...
			if (!found_first_empty && (swp->primary_bank_start_ammo[i] - swp->primary_bank_ammo[i])) {
				...
				prim_rearm_time +=
				    gamesnd_get_max_duration(gamesnd_get_game_sound(GameSounds::MISSILE_START_LOAD)) / 1000.0f;
			}
...
int ship_do_rearm_frame(object* objp, float frametime)
{
	...
	if (swp->num_primary_banks < sip->num_primary_banks &&
	    !aip->ai_profile_flags[AI::Profile_Flags::Support_dont_add_primaries]) {
	...
	if (!(objp->flags[Object::Object_Flags::No_shields])) {
		...
		if (shield_str < (max_shield_str)) {
			...
			shield_str += shipp->ship_max_shield_strength * frametime *
			              sip->sup_shield_repair_rate; // repair rate is unaffected by $Max Shield Recharge
			if (shield_str > max_shield_str) {
	...
	while (ssp != END_OF_LIST(&shipp->subsys_list)) {
		...
		if (ssp->current_hits < max_subsys_repair && repair_allocated > 0) {
			...
			if (!(ssp->flags[Ship::Subsystem_Flags::No_aggregate])) {
				...
				if (shipp->subsys_info[subsys_type].aggregate_current_hits >
				    shipp->subsys_info[subsys_type].aggregate_max_hits)
					shipp->subsys_info[subsys_type].aggregate_current_hits =
					    shipp->subsys_info[subsys_type].aggregate_max_hits;
			}
	...
	if (subsys_all_ok) {
		for (i = 0; i < swp->num_secondary_banks; i++) {
			...
			if (aip->rearm_first_missile == TRUE) {
				swp->secondary_bank_rearm_time[i] =
				    timestamp((int)gamesnd_get_max_duration(gamesnd_get_game_sound(GameSounds::MISSILE_START_LOAD)));
			}
	...
	if ((subsys_all_ok && shields_full && (The_mission.flags[Mission::Mission_Flags::Support_repairs_hull]) &&
	     hull_ok) ||
	    (subsys_all_ok && shields_full && !(The_mission.flags[Mission::Mission_Flags::Support_repairs_hull]))) {
		if (objp == Player_obj) {
...
// if no ships can satisfy a request, and we've reached the limits, but a request can be queued, return 3 and fill in
// the pointer if no ships can satisfy a request, we've reached the limits, and we can't queue anything, we're out of
// luck -- return 4
int ship_find_repair_ship(object* requester_obj, object** ship_we_found)
{
	...
	// no ships available; are we below the limits?  (can we bring another ship in? -- and btw an Arriving_support_ship
	// counts as being able to bring one in)
	else if ((num_support_ships < The_mission.support_ships.max_concurrent_ships) &&
	         ((Arriving_support_ship == NULL &&
	           The_mission.support_ships.tally < The_mission.support_ships.max_support_ships) ||
	          (Arriving_support_ship != NULL &&
	           The_mission.support_ships.tally <= The_mission.support_ships.max_support_ships))) {
	...
	while (moveup != END_OF_LIST(&sp->subsys_list)) {
		...
		if (moveup->current_hits > 0.0f) {
			if (moveup->system_info->alive_snd.isValid()) {
				obj_snd_assign(sp->objnum, moveup->system_info->alive_snd, &moveup->system_info->pnt, 0,
				               OS_SUBSYS_ALIVE, moveup);
				moveup->subsys_snd_flags.set(Ship::Subsys_Sound_Flags::Alive);
			}
			if (moveup->system_info->turret_base_rotation_snd.isValid()) {
				obj_snd_assign(sp->objnum, moveup->system_info->turret_base_rotation_snd, &moveup->system_info->pnt, 0,
				               OS_TURRET_BASE_ROTATION, moveup);
				moveup->subsys_snd_flags.set(Ship::Subsys_Sound_Flags::Turret_rotation);
			}
			if (moveup->system_info->turret_gun_rotation_snd.isValid()) {
				obj_snd_assign(sp->objnum, moveup->system_info->turret_gun_rotation_snd, &moveup->system_info->pnt, 0,
				               OS_TURRET_GUN_ROTATION, moveup);
				moveup->subsys_snd_flags.set(Ship::Subsys_Sound_Flags::Turret_rotation);
			}
			if ((moveup->system_info->rotation_snd.isValid()) && (moveup->flags[Ship::Subsystem_Flags::Rotates])) {
				obj_snd_assign(sp->objnum, moveup->system_info->rotation_snd, &moveup->system_info->pnt, 0,
				               OS_SUBSYS_ROTATION, moveup);
				moveup->subsys_snd_flags.set(Ship::Subsys_Sound_Flags::Rotate);
			}
		} else {
			if (moveup->system_info->dead_snd.isValid()) {
				obj_snd_assign(sp->objnum, moveup->system_info->dead_snd, &moveup->system_info->pnt, 0, OS_SUBSYS_DEAD,
				               moveup);
...
DCF(set_subsys, "Set the strength of a particular subsystem on player ship")
{
	...
	if (dc_optional_string_either("status", "--status") || dc_optional_string_either("?", "--?")) {
		dc_printf("Subsystem '%s' is at %f strength\n", arg.c_str(),
		          ship_get_subsystem_strength(Player_ship, subsystem));
...
int ship_query_general_type(int ship) { return ship_query_general_type(&Ships[ship]); }

int ship_query_general_type(ship* shipp) { return ship_class_query_general_type(shipp->ship_info_index); }
...
int ship_get_random_player_wing_ship(int flags, float max_dist, int persona_index, int get_first, int multi_team)
{
	...
	for (i = 0; i < Num_wings; i++) {
		...
		for (j = 0; j < Wings[wingnum].current_count; j++) {
			...
			if ((flags == SHIP_GET_NO_PLAYERS || flags == SHIP_GET_UNSILENCED) &&
			    (Objects[Ships[ship_index].objnum].flags[Object::Object_Flags::Player_ship])) {
				continue;
			}
...
//				do_facing_check	=>	OPTIONAL PARAMETER (default value is 1), do a dot product check to see if subsystem fvec
//is facing 											towards the eye position 				dot_out	=>		OPTIONAL PARAMETER, output parameter, will return dot between
//subsys fvec and subsys_to_eye_vec 									(only filled in if do_facing_check is true) 				vec_out	=>		OPTIONAL PARAMETER,
//vector from eye_pos to absolute subsys_pos.  (only filled in if do_facing_check is true)
int ship_subsystem_in_sight(object* objp, ship_subsys* subsys, vec3d* eye_pos, vec3d* subsys_pos, int do_facing_check,
                            float* dot_out, vec3d* vec_out)
{
	float dist, dot;
	mc_info mc;
...
std::string ship_return_orders(ship* sp)
{
	...
	case AI_GOAL_DESTROY_SUBSYSTEM: {
		if (aip->targeted_subsys != nullptr) {
			...
			core::sprintf(outbuf, XSTR("atk %s %s", 496), target_name.c_str(), subsys_name);
		} else {
			outbuf += XSTR("no orders", 495);
		}
		break;
...
void ship_maybe_warn_player(ship* enemy_sp, float dist)
{
	...
	if ((Builtin_messages[MESSAGE_CHECK_6].max_count > -1) &&
	    (Player->warn_count >= Builtin_messages[MESSAGE_CHECK_6].max_count)) {
		return;
	}
	...
	if (msg_type != -1) {
		...
		if (ship_index >= 0) {
			...
			if (Game_mode & GM_MULTIPLAYER) {
				message_send_builtin_to_player(msg_type, &Ships[ship_index], MESSAGE_PRIORITY_HIGH,
...
void ship_maybe_praise_player(ship* deader_sp)
{
	...
	if ((Builtin_messages[MESSAGE_PRAISE].max_count > -1) &&
	    (Player->praise_count >= Builtin_messages[MESSAGE_PRAISE].max_count)) {
		return;
	}
	...
	if ((Ship_info[deader_sp->ship_info_index].class_type > 0) &&
	    !(Ship_types[Ship_info[deader_sp->ship_info_index].class_type]
	          .flags[Ship::Type_Info_Flags::Praise_destruction])) {
		return;
	}
...
void ship_maybe_praise_self(ship* deader_sp, ship* killer_sp)
{
	...
	if ((Builtin_messages[MESSAGE_PRAISE_SELF].max_count > -1) &&
	    (Player->praise_self_count >= Builtin_messages[MESSAGE_PRAISE_SELF].max_count)) {
		return;
	}
	...
	message_send_builtin_to_player(MESSAGE_PRAISE_SELF, killer_sp, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_SOON, 0, 0, -1,
	                               -1);
...
static void awacs_maybe_ask_for_help(ship* sp, int multi_team_filter)
{
	...
	if (objp->hull_strength <
	    ((AWACS_HELP_HULL_LOW + 0.01f * (static_rand(OBJ_INDEX(objp)) & 5)) * sp->ship_max_hull_strength)) {
	...
	} else if (objp->hull_strength <
	           ((AWACS_HELP_HULL_HI + 0.01f * (static_rand(OBJ_INDEX(objp)) & 5)) * sp->ship_max_hull_strength)) {
	...
	if (message >= 0) {
		message_send_builtin_to_player(message, sp, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_IMMEDIATE, 0, 0, -1,
		                               multi_team_filter);
...
void ship_maybe_ask_for_help(ship* sp)
{
	...
	if ((Builtin_messages[MESSAGE_HELP].max_count > -1) &&
	    (Player->ask_help_count >= Builtin_messages[MESSAGE_HELP].max_count))
		return;
...
play_ask_help:
	...
	if (!(Ship_info[sp->ship_info_index]
	          .is_fighter_bomber())) // If we're still here, only continue if we're a fighter or bomber.
		return;

	if (!(sp->flags[Ship_Flags::No_builtin_messages])) // Karajorma - Only unsilenced ships should ask for help
	{
		message_send_builtin_to_player(MESSAGE_HELP, sp, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_IMMEDIATE, 0, 0, -1,
		                               multi_team_filter);
		Player->allow_ask_help_timestamp = timestamp(Builtin_messages[MESSAGE_HELP].min_delay);
...
void ship_maybe_lament()
{
	...
	if (rand() % 4 == 0) {
		ship_index = ship_get_random_player_wing_ship(SHIP_GET_UNSILENCED);
		if (ship_index >= 0)
			message_send_builtin_to_player(MESSAGE_PLAYER_DIED, &Ships[ship_index], MESSAGE_PRIORITY_HIGH,
			                               MESSAGE_TIME_IMMEDIATE, 0, 0, -1, -1);
	}
}
...
void ship_scream(ship* sp)
{
	...
	message_send_builtin_to_player(MESSAGE_WINGMAN_SCREAM, sp, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_IMMEDIATE, 0, 0, -1,
	                               multi_team_filter);
...
void ship_maybe_scream(ship* sp)
{
	...
	if (!(sp->flags[Ship_Flags::Always_death_scream])) {
		...
		if (!(The_mission.ai_profile->flags[AI::Profile_Flags::Perform_fewer_scream_checks])) {
			...
			if ((Builtin_messages[MESSAGE_WINGMAN_SCREAM].max_count > -1) &&
			    (Player->scream_count >= Builtin_messages[MESSAGE_WINGMAN_SCREAM].max_count)) {
				return;
			}
...
/**
 * This function is only for notifying the player that the ship's ammo is low, without necessarily requesting support
 * (e.g. if support is disallowed for this mission).  It is not called if support was successfully requested in
		...
		if (wip->wi_flags[Weapon::Info_Flags::Ballistic]) {
			if (swp->primary_bank_start_ammo[i] > 0) {
				if ((float)swp->primary_bank_ammo[i] / (float)swp->primary_bank_start_ammo[i] < 0.3f) {
					...
					message_send_builtin_to_player(MESSAGE_PRIMARIES_LOW, sp, MESSAGE_PRIORITY_NORMAL,
...
void ship_maybe_tell_about_rearm(ship* sp)
{
	...
	if (message_type >= 0) {
		if (rand() & 1)
			message_send_builtin_to_player(message_type, sp, MESSAGE_PRIORITY_NORMAL, MESSAGE_TIME_SOON, 0, 0, -1,
			                               multi_team_filter);
	...
	if (sp->flags[Ship_Flags::Primary_linked]) {
		...
		for (i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) {
			if (i == swp->current_primary_bank) {
				...
				if (swp->primary_animation_position[i] == MA_POS_NOT_SET) {
					if (model_anim_start_type(sp, TRIGGER_TYPE_PRIMARY_BANK, i, 1)) {
						swp->primary_animation_done_time[i] =
						    model_anim_get_time_type(sp, TRIGGER_TYPE_PRIMARY_BANK, i);
						swp->primary_animation_position[i] = MA_POS_SET;
					} else {
						swp->primary_animation_position[i] = MA_POS_READY;
					}
			...
			if (i == swp->current_secondary_bank) {
				...
				if (swp->secondary_animation_position[i] == MA_POS_NOT_SET) {
					if (model_anim_start_type(sp, TRIGGER_TYPE_SECONDARY_BANK, i, 1)) {
						swp->secondary_animation_done_time[i] =
						    model_anim_get_time_type(sp, TRIGGER_TYPE_SECONDARY_BANK, i);
...
flagset<Ship::Info_Flags> ship_get_SIF(ship* shipp) { return Ship_info[shipp->ship_info_index].flags; }

flagset<Ship::Info_Flags> ship_get_SIF(int sh) { return Ship_info[Ships[sh].ship_info_index].flags; }
 ...
 *
 * Happens at two different locations (at least when this function was written), one for the player, and one for AI
 * ships. Need to send stuff to clients in multiplayer game.
 */
...
void ship_do_cap_subsys_cargo_revealed(ship* shipp, ship_subsys* subsys, int from_network)
{
	...
	if (stricmp(Cargo_names[subsys->subsys_cargo_name & CARGO_INDEX_MASK], NOX("nothing")) != 0) {
		mission_log_add_entry(LOG_CAP_SUBSYS_CARGO_REVEALED, shipp->ship_name, subsys->system_info->subobj_name,
...
void ship_page_in()
{
	...
	core::Verify(ship_class_used != NULL);
	...
	if (!ship_allocate_subsystems(num_subsystems_needed, true)) {
		core::Error(LOCATION,
		      "Attempt to page in new subsystems subsystems failed, which shouldn't be possible anymore. Currently "
		      "allocated %d subsystems (%d in use)",
		      Num_ship_subsystems_allocated, Num_ship_subsystems);
	}
			...
			if (ship_previously_loaded != i) {
				...
				for (j = 0; j < sip->n_subsystems; j++) {
					if (sip->subsystems[j].model_num != sip->model_num) {
						...
						polymodel* subsys_pm =
						    (sip->subsystems[j].model_num >= 0) ? model_get(sip->subsystems[j].model_num) : NULL;
						core::Warning(LOCATION,
						        "After ship_copy_subsystem_fixup, ship '%s' does not have subsystem '%s' linked into "
						        "the model file, '%s'.\n\n(Ship_info model is '%s' and subsystem model is '%s'.)",
						        sip->name, sip->subsystems[j].subobj_name, sip->pof_file,
				...
				for (j = 0; j < sip->n_subsystems; j++) {
					if (sip->subsystems[j].model_num != sip->model_num) {
						...
						polymodel* subsys_pm =
						    (sip->subsystems[j].model_num >= 0) ? model_get(sip->subsystems[j].model_num) : NULL;
						core::Warning(LOCATION,
						        "Without ship_copy_subsystem_fixup, ship '%s' does not have subsystem '%s' linked into "
						        "the model file, '%s'.\n\n(Ship_info model is '%s' and subsystem model is '%s'.)",
						        sip->name, sip->subsystems[j].subobj_name, sip->pof_file,
						        (sip_pm != NULL) ? sip_pm->filename : "NULL",
						        (subsys_pm != NULL) ? subsys_pm->filename : "NULL");
	...
	if (Game_mode & GM_MULTIPLAYER) {
		for (i = 0; i < MAX_PLAYERS; i++) {
			if (MULTI_CONNECTED(Net_players[i]) && (Net_players[i].m_player != NULL) &&
...
int is_support_allowed(object* objp, bool do_simple_check)
{
	...
	if (!do_simple_check) {
		...
		if (The_mission.support_ships.max_support_ships > 0) {
			// if all the allowed ships have been used up, can't rearm unless something's available in-mission or
			// arriving
	...
	if (!do_simple_check) {
		...
		if (model_find_dock_index(Ship_info[shipp->ship_info_index].model_num, DOCK_TYPE_REARM) < 0) {
			...
			if (!warned_about_rearm_dockpoint) {
				core::Warning(LOCATION, "Support not allowed for %s because its model lacks a rearming dockpoint!",
				        shipp->ship_name);
...
void object_jettison_cargo(object* objp, object* cargo_objp, float jettison_speed, bool jettison_new)
{
	...
	if (jettison_new) {
		...
		extern void find_adjusted_dockpoint_normal(vec3d * global_p0_norm, object * objp, polymodel * pm, int submodel,
		                                           int dock_index);
...
static int ship_get_exp_propagates(ship* sp) { return Ship_info[sp->ship_info_index].explosion_propagates; }
...
int ship_get_reinforcement_team(int r_index)
{
	...
	if (wing_index >= 0) {
		...
		for (p_objp = GET_FIRST(&Ship_arrival_list); p_objp != END_OF_LIST(&Ship_arrival_list);
		     p_objp = GET_NEXT(p_objp)) {
			// check by wingnum
...
#define CLEAR_ARTILLERY_AND_CONTINUE()                                                                                 \
	{                                                                                                                  \
		if (aip != NULL) {                                                                                             \
...
DCF(art, "Sets artillery disance") { dc_stuff_float(&artillery_dist); }
	...
	for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
		...
		Assert(
		    ...
		    (Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]].b_info.beam_type ==
		     BEAM_TYPE_C));
		if (!(Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]]
		          .wi_flags[Weapon::Info_Flags::Beam]) ||
		    (Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]].b_info.beam_type !=
		     BEAM_TYPE_C)) {
			continue;
...
int check_world_pt_in_expanded_ship_bbox(vec3d* world_pt, object* objp, float delta_box)
{
	...
	return ((ship_pt.xyz.x > pm->mins.xyz.x - delta_box) && (ship_pt.xyz.x < pm->maxs.xyz.x + delta_box) &&
...
/**
 * Get the offset of the actual center of the ship model for the purposes of warping (which may not be the specified
 * center)
 */
...
void ship_do_submodel_rotation(ship* shipp, model_subsystem* psub, ship_subsys* pss)
{
	...
	if (psub->flags[Model::Subsystem_Flags::Artillery]) {
		...
		// check unlinked
			...
			if ((psub->weapon_rotation_pbank != swp->current_primary_bank) ||
			    (swp->primary_bank_ammo[swp->current_primary_bank] <= 0))
				return;
		}
	}
...
// Goober5000
int ship_has_engine_power(ship* shipp) { return (Ship_info[shipp->ship_info_index].max_speed > 0); }
...
static int ship_class_get_priority(int ship_class)
{
	...
	core::Warning(LOCATION, "Unknown priority for ship class '%s'!", sip->name);
...
int damage_type_add(char* name)
{
	...
	if (strlen(name) > NAME_LENGTH - 1) {
		core::Warning(LOCATION, "Damage type name '%s' is too long and has been truncated to '%s'", name, dts.name);
	}
...
const char* TypeNames[] = {"additive", "multiplicative", "exponential", "exponential base", "cutoff", "reverse cutoff",
                           "instant cutoff", "instant reverse cutoff",
                           // Added by Nuke
                           "cap", "instant cap", "set", "load", "store", "random"};
...
float ArmorType::GetDamage(float damage_applied, int in_damage_type_idx, float diff_dmg_scale, int is_beam)
{
	...
	if (adtp != NULL) {
		...
		for (size_t i = 0; i < num; i++) {
			...
			} else if (storage_idx == AT_CONSTANT_NOT_USED) { // save time checking all possible constants when most of
				                                              // the time you will be using +value:
			...
			if (is_beam &&
			    (adtp->Calculations[i] == AT_TYPE_ADDITIVE || adtp->Calculations[i] == AT_TYPE_CUTOFF ||
			     adtp->Calculations[i] == AT_TYPE_REVERSE_CUTOFF || adtp->Calculations[i] == AT_TYPE_INSTANT_CUTOFF ||
			     adtp->Calculations[i] == AT_TYPE_INSTANT_REVERSE_CUTOFF || adtp->Calculations[i] == AT_TYPE_CAP ||
			     adtp->Calculations[i] == AT_TYPE_INSTANT_CAP || adtp->Calculations[i] == AT_TYPE_SET ||
			...
			case AT_TYPE_STORE:
				if (using_storage || using_constant) {
					core::Warning(LOCATION, "Cannot use +Stored Value: or +Constant: with +Store:, that would be bad. "
					...
					if ((storage_idx < 0) || (storage_idx >= AT_NUM_STORAGE_LOCATIONS)) {
						core::Warning(LOCATION,
						        "+Value: for +Store: calculation out of range. Should be between 0 and %i. Read: %i, "
						        "Skipping calculation.",
			...
			case AT_TYPE_LOAD:
				if (using_storage || using_constant) {
					core::Warning(LOCATION, "Cannot use +Stored Value: or +Constant: with +Load:, that would be bad. "
					                  "Skipping calculation.");
				} else {
					storage_idx = int(floorf(curr_arg));
					// Nuke: idiotproof this, no segfault 4 u
					if ((storage_idx < 0) || (storage_idx >= AT_NUM_STORAGE_LOCATIONS)) {
						core::Warning(LOCATION,
						        "+Value: for +Load: calculation out of range. Should be between 0 and %i. Read: %i, "
						        "Skipping calculation.",
						        AT_NUM_STORAGE_LOCATIONS, storage_idx);
						storage_idx = 0;
					} else {
...
ArmorType::ArmorType(const char* in_name)
{
	...
	if (len >= NAME_LENGTH) {
		core::Warning(LOCATION, "Armor name %s is " SIZE_T_ARG " characters too long, and will be truncated", in_name,
		        len - NAME_LENGTH);
	}
...
void ArmorType::ParseData()
{
	...
	do {
		...
		while (optional_string("+Calculation:")) {
			...
			if (calc_type == -1) {
				core::Warning(LOCATION, "Armor '%s': Armor calculation type '%s' is invalid, and has been skipped", Name,
				        buf);
				...
				if (optional_string("+Stored Value:")) {
					...
					if ((temp_int < 0) || (temp_int >= AT_NUM_STORAGE_LOCATIONS)) {
						core::Error(LOCATION,
						      "+Stored Value: is out of range. Should be between 0 and %i. Read: %i, Using value 0.",
						      AT_NUM_STORAGE_LOCATIONS - 1, temp_int);
						temp_int = AT_CONSTANT_NOT_USED;
					}
					adt.altArguments.push_back(temp_int);
					adt.Arguments.push_back(0.0f); // this isnt used in this case, just take up space so the indices
					                               // lign up, also a fallback value in case of bad altArguments
					...
					if (temp_int == AT_CONSTANT_BAD_VAL) {
						core::Error(LOCATION, "Invalid +Constant: name, '%s'. Using value 0.", buf);
						temp_int = AT_CONSTANT_NOT_USED;
					}
					adt.altArguments.push_back(temp_int);
					adt.Arguments.push_back(0.0f); // this isnt used in this case, just take up space so the indices
					...
					required_string("+Value:");
					stuff_float(&temp_float);
					adt.altArguments.push_back(
					    AT_CONSTANT_NOT_USED); // set this to AT_CONSTANT_NOT_USED so we know to just use the value from
					                           // adt.Arguments instead of constants/storage locations
		...
		if (optional_string("+Difficulty Scale Type:")) {
			...
			if (temp_int == ADT_DIFF_SCALE_BAD_VAL) {
				core::Error(LOCATION, "Invalid +Difficulty Scale Type: name: '%s'. Reverting to default behavior.", buf);
				adt.difficulty_scale_type = ADT_DIFF_SCALE_FIRST;
			} else {
		...
		if (!no_content) {
			if (adt.Calculations.size() != adt.Arguments.size()) {
				core::Warning(LOCATION,
				        ...
				        ": Armor has a different number of calculation types than arguments (" SIZE_T_ARG
				        ", " SIZE_T_ARG ")",
	...
	if (optional_string("+Weapon Class:")) {
		...
		for (i = 0; i < num_strings; i++) {
			...
			if (j == MAX_WEAPON_TYPES) {
				core::Warning(LOCATION, "Unidentified weapon class '%s' set for target priority group '%s'\n",
	...
	if (optional_string("+Object Flags:")) {
		...
		for (i = 0; i < num_strings; i++) {
			...
			if (j == num_ai_tgt_obj_flags) {
				core::Warning(LOCATION, "Unidentified object flag '%s' set for target priority group '%s'\n",
				        temp_strings[i].c_str(), temp_priority.name);
			}
		}
	}
	...
	if (optional_string("+Ship Class Flags:")) {
		...
		for (i = 0; i < num_strings; i++) {
			...
			if (j == num_ai_tgt_ship_flags) {
				core::Warning(LOCATION, "Unidentified ship class flag '%s' set for target priority group '%s'\n",
				        temp_strings[i].c_str(), temp_priority.name);
			}
		}
	}
	...
	if (optional_string("+Weapon Class Flags:")) {
		...
		for (i = 0; i < num_strings; i++) {
			...
			if (j == num_ai_tgt_weapon_info_flags) {
				core::Warning(LOCATION, "Unidentified weapon class flag '%s' set for target priority group '%s'\n",
				        temp_strings[i].c_str(), temp_priority.name);
			}
		}
	}
		...
		if (render_amount > 0.0f) {
			...
			if (shipp->thrusters_start[i] <= 0) {
				...
				if (mtp->start_snd.isValid())
					snd_play_3d(gamesnd_get_game_sound(mtp->start_snd), &mtp->pos, &Eye_position, 0.0f,
					            &obj->phys_info.vel);
			}
			...
			if (!Cmdline_freespace_no_sound) {
				if (mtp->loop_snd.isValid() && shipp->thrusters_sounds[i] < 0 &&
				    (!mtp->start_snd.isValid() || (gamesnd_get_max_duration(gamesnd_get_game_sound(mtp->start_snd)) <
				                                   timestamp() - shipp->thrusters_start[i]))) {
			...
			if (mtp->tex_id >= 0) {
				...
				if (mtp->tex_nframes > 0)
					bmap_frame += bm_get_anim_frame(
					    mtp->tex_id, i2fl(timestamp() - shipp->thrusters_start[i]) / 1000.0f, 0.0f, true);
                               ...
                               int render_flags)
{
	...
	for (i = 0; i < swp->num_primary_banks; i++) {
		...
		for (k = 0; k < bank->num_slots; k++) {
			...
			model_render_queue(ship_render_info, scene, Weapon_info[swp->primary_bank_weapons[i]].external_model_num,
			                   &vmd_identity_matrix, &bank->pnt[k]);
	...
	for (i = 0; i < swp->num_secondary_banks; i++) {
		...
		if (Weapon_info[swp->secondary_bank_weapons[i]].wi_flags[Weapon::Info_Flags::External_weapon_lnch]) {
			for (k = 0; k < bank->num_slots; k++) {
				model_render_queue(ship_render_info, scene,
				                   Weapon_info[swp->secondary_bank_weapons[i]].external_model_num, &vmd_identity_matrix,
				                   &bank->pnt[k]);
			}
		} else {
			...
			for (k = 0; k < bank->num_slots; k++) {
				...
				vm_vec_scale_add2(&secondary_weapon_pos, &vmd_z_vector,
				                  -(1.0f - shipp->secondary_point_reload_pct[i][k]) *
				                      model_get(Weapon_info[swp->secondary_bank_weapons[i]].external_model_num)->rad);
...
int ship_render_get_insignia(object* obj, ship* shipp)
{
	...
	if (Game_mode & GM_MULTIPLAYER) {
		...
		if ((np_index >= 0) && (np_index < MAX_PLAYERS) && MULTI_CONNECTED(Net_players[np_index]) &&
...
void ship_render_set_animated_effect(model_render_params* render_info, ship* shipp, uint* /*render_flags*/)
{
	...
	if (sep->invert_timer) {
		timer = 1.0f -
		        ((timer_get_milliseconds() - shipp->shader_effect_start_time) / (float)shipp->shader_effect_duration);
	...
	if (sep->disables_rendering &&
	    (timer_get_milliseconds() > shipp->shader_effect_start_time + shipp->shader_effect_duration)) {
...
void ship_render(object* obj, model_draw_list* scene)
{
	...
	if (sip->uses_team_colors && !shipp->flags[Ship_Flags::Render_without_miscmap]) {
		...
		bool team_color_set = model_get_team_color(&model_team_color, shipp->team_name, shipp->secondary_team_name,
		                                           shipp->team_change_timestamp, shipp->team_change_time);
	...
	if (stage == ship::warpstage::BOTH) {
		if (!dock_leader_or_single) {
			return flags[Ship::Ship_Flags::Arriving_stage_1, Ship::Ship_Flags::Arriving_stage_1_dock_follower,
			             Ship::Ship_Flags::Arriving_stage_2, Ship::Ship_Flags::Arriving_stage_2_dock_follower];
		} else {
