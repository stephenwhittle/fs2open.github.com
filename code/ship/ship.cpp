...
float ship_calculate_rearm_duration( object *objp )
{
	...
	for (i = 0; i < swp->num_primary_banks; i++)
	{
		...
		if (wip->wi_flags[Weapon::Info_Flags::Ballistic])
		{
			...
			if (!found_first_empty && (swp->primary_bank_start_ammo[i] - swp->primary_bank_ammo[i]))
			{
				...
				prim_rearm_time += gamesnd_get_max_duration(&Snds[SND_MISSILE_START_LOAD]) / 1000.0f;
			}
	...
	for (i = 0; i < swp->num_secondary_banks; i++)
	{
			...
			{
				found_first_empty = true;
				sec_rearm_time += gamesnd_get_max_duration(&Snds[SND_MISSILE_START_LOAD]) / 1000.0f;
			}
...
//	Comments removed by PhReAk; Note that this is toggled on/off with a mission flag
	...
	if ( subsys_all_ok )
	{
		for (i = 0; i < swp->num_secondary_banks; i++ )
		{
			...
			{
				swp->secondary_bank_rearm_time[i] = timestamp((int)gamesnd_get_max_duration(&Snds[SND_MISSILE_START_LOAD]));
			}
		...
		for (i = 0; i < swp->num_primary_banks; i++ )
		{
			if (Weapon_info[swp->primary_bank_weapons[i]].wi_flags[Weapon::Info_Flags::Ballistic])
			{
				...
				if ( aip->rearm_first_ballistic_primary == TRUE )
				{
					...
					swp->primary_bank_rearm_time[i] = timestamp((int)gamesnd_get_max_duration(&Snds[sound_index]));
				}
				...
				if ( swp->primary_bank_ammo[i] < swp->primary_bank_start_ammo[i] )
				{
					...
					if ( timestamp_elapsed(swp->primary_bank_rearm_time[i]) )
					{
						...
						// Goober5000
						int sound_index;
						if (Snds.size() > SND_BALLISTIC_LOAD)
		...
		if ( render_amount > 0.0f ) {
			...
			if (!Cmdline_freespace_no_sound) {
				if(mtp->loop_snd >= 0
					...
					&& (mtp->start_snd < 0 || (gamesnd_get_max_duration(&Snds[mtp->start_snd]) < timestamp() - shipp->thrusters_start[i]))
					)
				{
