...
{	
	...
	core::Assertion(ps->ping_times[ps->ping_add-1] > 0L, "Non-positive ping value!");
		ps->num_pings++;
	} 
	// otherwise if we've wrapped around
		...
		ps->ping_times[ps->ping_add] = pong_time - ps->ping_start;
	core::Assertion(ps->ping_times[ps->ping_add == 0 ? MAX_PINGS - 1 : ps->ping_add - 1] > 0L, "Non-positive ping value!");
	}
