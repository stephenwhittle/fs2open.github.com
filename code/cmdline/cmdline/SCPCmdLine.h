...
public:
	...
	SCPCmdLineOption(const char* Name, const char* HelpText, bool FSOOnly,
					 OptionsCategory Category = OptionsCategory::Misc, FlagGroup Group = FlagGroup::None)
		: tl::optional<T>(),
		  Name(Name),
		  HelpText(HelpText),
		  FSOOnly(FSOOnly),
	...
	auto operator==(const SCPCmdLineOption& Other) const
	{
		return *this == Other;
	}
};
...
public:
	...
	// So long as you want all the options to be universally accessible these will be actual variables stored here
	// rather than stored in their respective modules Retail Options
	SCPCmdLineOption<std::string> AutoConnectAddress = {
		"Autoconnect Address", "Automatically connect to multiplayer IP:PORT", false, OptionsCategory::Multiplayer};
	SCPCmdLineOption<std::string> MultiplayerGameName = {"Game Name", "Set multiplayer game name", false,
														 OptionsCategory::Multiplayer};
	SCPCmdLineOption<std::string> MultiplayerGamePassword = {"Game Password", "Set multiplayer game password", false,
															 OptionsCategory::Multiplayer};
	SCPCmdLineOption<std::string> MultiplayerMinimumRank = {"Minimum Rank", "Ranks above this can join multi", false,
															OptionsCategory::Multiplayer};
	SCPCmdLineOption<std::string> MultiplayerMaximumRank = {"Maximum Rank", "Ranks below this can join multi", false,
															OptionsCategory::Multiplayer};
	SCPCmdLineOption<bool> bStandalone = {"Standalone Mode", "Run as standalone server", false,
										  OptionsCategory::Multiplayer};
	...
	SCPCmdLineOption<bool> bNoEnhancedSound = {"Disable Enhanced Sound", "Disable enhanced sound", false,
											   OptionsCategory::Audio};
	SCPCmdLineOption<bool> bStartGame = {"Start Game", "Skip main menu and start hosting", false,
										 OptionsCategory::Multiplayer};
	SCPCmdLineOption<bool> bClosedGame = {"Closed Game", "Start hosted server as closed", false,
										  OptionsCategory::Multiplayer};
	SCPCmdLineOption<bool> bRestrictedGame = {"Restricted Mode", "Host must confirm all join requests", false,
											  OptionsCategory::Multiplayer};
	SCPCmdLineOption<int> MultiplayerPort = {"Multiplayer Port", "Multiplayer network port", false,
											 OptionsCategory::Multiplayer};
	...
	SCPCmdLineOption<bool> bPofSpew = {"pofspew", "Dump model information to pofspew.txt", false,
									   OptionsCategory::DevTool};
	SCPCmdLineOption<bool> bWeaponSpew = {"weaponspew", "Dump weapon stats and spreadsheets", false,
										  OptionsCategory::DevTool};
	SCPCmdLineOption<bool> bMouseCoords = {"coords", "Show coordinates", false, OptionsCategory::DevTool};
	SCPCmdLineOption<int> MultiplayerTimeout = {"Multiplayer Timeout", "Multiplayer network timeout (seconds)", false,
												OptionsCategory::Multiplayer};
	...
	SCPCmdLineOption<float> ClipDistance = {"Clip Distance",
											"Changes the distance from the viewport for the near-clipping plane", true,
											OptionsCategory::Graphics};
	SCPCmdLineOption<float> SpecStatic = {"Sun Specular Contribution",
										  "Adjusts sun contribution to specular highlights", true,
										  OptionsCategory::Graphics};
	SCPCmdLineOption<float> SpecPoint = {"Laser Specular Contribution",
										 "Adjusts laser weapon contribution to specular highlights", true,
										 OptionsCategory::Graphics};
	SCPCmdLineOption<float> SpecTube = {"Beam Weapon Specular Contribution",
										"Adjusts beam weapons contribution to specular highlights", true,
										OptionsCategory::Graphics};
	SCPCmdLineOption<float> AmbientFactor = {"Ambient light factor",
											 "Adjusts ambient light applied to all parts of a ship", true,
											 OptionsCategory::Graphics};
	...
	SCPCmdLineOption<bool> bNoMotionDebris = {"No motion debris", "Removes floating debris", true,
											  OptionsCategory::Graphics};
	SCPCmdLineOption<bool> bNoScaleVid = {"No video scale", "Do not scale videos to fit the screen resolution", true,
										  OptionsCategory::Graphics};
	SCPCmdLineOption<bool> bNoSpec = {"No specular", "deprecated", true, OptionsCategory::Graphics};
	SCPCmdLineOption<bool> bEmissive = {"Emissive", "Enable emissive light from ships", true,
										OptionsCategory::Graphics};
	...
	SCPCmdLineOption<bool> bNoPostProcess = {"No post-processing", "Disables post-processing", true,
											 OptionsCategory::Graphics};
	SCPCmdLineOption<int> BloomIntensity = {"Bloom Intensity", "Bloom intensity (requires post-processing)", true,
											OptionsCategory::Graphics};
	SCPCmdLineOption<bool> bPostAA = {"Enable Anti-aliasing", "Enable post-process AA", true,
									  OptionsCategory::Graphics};
	SCPCmdLineOption<int> AAPreset = {"AA Preset", "Sets AA type/level, see wiki for values", true,
									  OptionsCategory::Graphics};
	...
	SCPCmdLineOption<bool> bDisableAutopilotInterrupt = {"Allow interrupting autopilot", "", true,
														 OptionsCategory::Gameplay};
	...
	SCPCmdLineOption<std::string> ModList = {"Mods", "List of folders to overlay over default data", true,
											 OptionsCategory::Misc};

	SCPCmdLineOption<std::string> AutoloadMission = {"Autoload multiplayer mission",
													 "Name of mission to automatically load for multiplayer", true,
													 OptionsCategory::Multiplayer};
	SCPCmdLineOption<bool> bAllowIngameJoin = {"In-game join", "", true, OptionsCategory::Multiplayer};
	SCPCmdLineOption<bool> bDisallowReturn = {"Disallow return",
											  "Hides 'Return to flight deck' option in respawn dialog", true,
											  OptionsCategory::Multiplayer};
	SCPCmdLineOption<int> ObjectUpdateCap = {"Object update cap", "Cap multiplayer object updates (0-3)", true,
											 OptionsCategory::Multiplayer};
	...
	SCPCmdLineOption<std::string> KeyboardLayout = {"Keyboard Layout",
													"Specify keyboard layout (\"qwertz\" or \"azerty\")", true,
													OptionsCategory::Troubleshooting};
	...
	SCPCmdLineOption<std::string> AutoStartMission = {"Start Mission", "Skip mainhall and run this mission", true,
													  OptionsCategory::DevTool};
	...
	SCPCmdLineOption<bool> bNoParseErrors = {"No Parse Errors", "Disables parser error notification", true,
											 OptionsCategory::DevTool};
	...
	SCPCmdLineOption<std::string> CenterMonitorResolution = {"Center Monitor Resolution", "", true,
															 OptionsCategory::DevTool};
	...
	SCPCmdLineOption<bool> bNoUnfocusedPause = {"No Unfocused Pause", "Don't pause the game when it loses focus", true,
												OptionsCategory::DevTool};
	...
	SCPCmdLineOption<bool> bEnableGraphicsDebugOutput = {"Enable Graphics debug output", "", true,
														 OptionsCategory::DevTool};
	SCPCmdLineOption<bool> bLogToStdout = {"Log to stdout", "", true, OptionsCategory::DevTool};

	SCPCmdLineOption<std::string> FlagsFilePath = {"Output launcher flags", "Save the launcher flags to file", true,
												   OptionsCategory::Misc};
	SCPCmdLineOption<bool> bOutputSEXPs = {"Output SEXPs", "Saves all SEXPs to sexps.html", true,
										   OptionsCategory::Misc};
