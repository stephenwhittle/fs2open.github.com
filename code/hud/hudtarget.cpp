...
float Offscreen_tri_base[GR_NUM_RESOLUTIONS]           = {6.0f, 9.5f};
float Offscreen_tri_height[GR_NUM_RESOLUTIONS]         = {7.0f, 11.0f};
float Max_offscreen_tri_seperation[GR_NUM_RESOLUTIONS] = {10.0f, 16.0f};
float Max_front_seperation[GR_NUM_RESOLUTIONS]         = {10.0f, 16.0f};
...
float Target_triangle_base[GR_NUM_RESOLUTIONS]   = {6.0f, 9.5f};
float Target_triangle_height[GR_NUM_RESOLUTIONS] = {7.0f, 11.0f};
...
void HudGaugeWeapons::render(float /*frametime*/)
{
	...
	for (i = 0; i < np; i++) {
		...
		events::RenderPrimaryWeapon(i, name, ammo_str, renderLinkIcon);
		if (i != 0) {
			y += primary_text_h;
		}
	...
	for (i = 0; i < ns; i++) {
		...
		events::RenderSecondaryWeapon(i, name, ammo_str, armed, linked, cooldown );
		if (i != 0)
		{
			y += secondary_text_h;
		}
