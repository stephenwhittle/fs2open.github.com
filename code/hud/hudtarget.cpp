...
char Lead_fname[NUM_HUD_RETICLE_STYLES][GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {{"lead1_fs1", "2_lead1_fs1"},
                                                                                 {"lead1", "2_lead1"}};
...
int Cm_coords[GR_NUM_RESOLUTIONS][2]                = {{// GR_640
                                        ...
                                        {// GR_1024
                                         880, 602}};
int Cm_text_coords[GR_NUM_RESOLUTIONS][2]           = {{// GR_640
                                              533, 347},
                                             {// GR_1024
                                              916, 606}};
int Cm_text_val_coords[GR_NUM_RESOLUTIONS][2]       = {{// GR_640
                                                  506, 347},
                                                 {// GR_1024
                                                  889, 606}};
char Cm_fname[GR_NUM_RESOLUTIONS][MAX_FILENAME_LEN] = {"countermeasure1", "countermeasure1"};
...
int Weapon_title_coords[NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS][2] = {{ // normal HUD
                                                                     {// GR_640
                                                                      518, 274},
                                                                     {// GR_1024
                                                                      901, 527}},
...
int Weapon_slinked_x[GR_NUM_RESOLUTIONS]   = {525, // where to draw the second thingie if this weapon is fire-linked
                                            908};
...
int Weapon_secondary_name_x[GR_NUM_RESOLUTIONS]   = {536, // x location of where to draw weapon name
                                                   919};
int Weapon_secondary_ammo_x[GR_NUM_RESOLUTIONS]   = {525, // x location of where to draw weapon ammo count
                                                   908};
int Weapon_primary_ammo_x[GR_NUM_RESOLUTIONS]     = {525, // x location of where to draw primary weapon ammo count
                                                 908};
int Weapon_secondary_reload_x[GR_NUM_RESOLUTIONS] = {615, // x location of where to draw the weapon reload time
                                                     998};
const char* Weapon_gauge_fnames[NUM_HUD_SETTINGS][GR_NUM_RESOLUTIONS][NUM_WEAPON_GAUGES] = {
    ...
    { // ballistic HUD - slightly different alignment
     {// GR_640
      "weapons1_b", "weapons2_b", "weapons3_b", "weapons4_b", "weapons5_b"},
     {// GR_1024
      "weapons1_b", "weapons2_b", "weapons3_b", "weapons4_b", "weapons5_b"}}
    // XSTR:ON
};
...
{
core::Assert((Player_ai->target_objnum >= 0) && (Player_ai->target_objnum < MAX_OBJECTS));
	if (!((Player_ai->target_objnum >= 0) && (Player_ai->target_objnum < MAX_OBJECTS))) {
		return;
	}
core::Assert(Objects[Player_ai->target_objnum].type == OBJ_SHIP);
...
object* hud_reticle_pick_target()
{
	...
	for (cur_rl = GET_FIRST(&Reticle_cur_list); cur_rl != END_OF_LIST(&Reticle_cur_list); cur_rl = GET_NEXT(cur_rl)) {
		in_save_list = 0;
		for (save_rl = GET_FIRST(&Reticle_save_list); save_rl != END_OF_LIST(&Reticle_save_list);
		     save_rl = GET_NEXT(save_rl)) {
...
core::Assert(target != END_OF_LIST(plist));
...
void hud_weapons_init()
{
	...
	// The E: There used to be a number of checks here. They are no longer needed, as the new HUD code handles it on its
	// own.
...
void hud_init_homing_beep()
{
	...
	Homing_beep.precalced_interp = (Homing_beep.max_cycle_dist - Homing_beep.min_cycle_dist) /
	                               (Homing_beep.max_cycle_time - Homing_beep.min_cycle_time);
}
...
void hud_target_prev_subobject() { hud_target_subobject_common(0); }

void hud_target_next_subobject() { hud_target_subobject_common(1); }
...
void hud_target_common(int team_mask, int next_flag)
{
	...
	for (A = start2; A != start; A = advance_fb(A, next_flag)) {
		...
		core::Assertion(jnp != Jump_nodes.end(), "Failed to find jump node with object index %d; trace out and fix!\n",
			          OBJ_INDEX(A));
...
void hud_target_next(int team_mask) { hud_target_common(team_mask, 1); }

void hud_target_prev(int team_mask) { hud_target_common(team_mask, 0); }
...
static object* select_next_target_by_distance(const bool targeting_from_closest_to_farthest, const int valid_team_mask,
                                              const int attacked_object_number          = -1,
                                              flagset<Ship::Info_Flags>* target_filters = NULL)
{
	...
	for (ship_object_ptr = GET_FIRST(&Ship_obj_list); ship_object_ptr != END_OF_LIST(&Ship_obj_list);
	     ship_object_ptr = GET_NEXT(ship_object_ptr)) {
		...
		if (attacked_object_number == -1) {
			...
			if ((attacked_object_number == player_object_index) &&
			    (Ai_info[prospective_victim_ship_ptr->ai_index].target_objnum != player_object_index)) {
				continue;
			}
...
core::Assert(Ships[source_obj->instance].ai_index != -1);
	...
	if (aip->target_objnum != -1) {
		...
		if (target_objp->type == OBJ_WEAPON &&
		    Weapon_info[Weapons[target_objp->instance].weapon_info_index].subtype == WP_MISSILE) { // must be a missile
	...
	if (!target_found) {
		...
		for (so = advance_ship(startShip, next_flag); so != startShip; so = advance_ship(so, next_flag)) {
			...
			if (!(Ship_info[Ships[A->instance].ship_info_index].flags[Ship::Info_Flags::Bomber]) &&
			    !(A->flags[Object::Object_Flags::Targetable_as_bomb])) {
				continue;
			}
...
core::Assert(p1->type != TYPE_NONE);
core::Assert(p2->type != TYPE_NONE);
...
void hud_target_live_turret(int next_flag, int auto_advance, int only_player_target)
{
	...
	if (Player_ai->targeted_subsys != NULL) {
		if (Player_ai->targeted_subsys->system_info->type == SUBSYSTEM_TURRET) {
			if (Player_ai->targeted_subsys->weapons.num_primary_banks > 0 ||
			    Player_ai->targeted_subsys->weapons.num_secondary_banks > 0) {
...
core::Assert(objp->type == OBJ_SHIP);
	...
	// bail if harmless
	if (Ship_info[esct_p->shipp->ship_info_index].class_type > -1 &&
	    !(Ship_types[Ship_info[esct_p->shipp->ship_info_index].class_type]
	          .flags[Ship::Type_Info_Flags::Target_as_threat])) {
		return false;
	}
	...
	if (Ship_info[esct_p->shipp->ship_info_index].is_big_or_huge()) {
		for (ss = GET_FIRST(&esct_p->shipp->subsys_list); ss != END_OF_LIST(&esct_p->shipp->subsys_list);
		     ss = GET_NEXT(ss)) {
			...
			if ((ss->system_info->type == SUBSYSTEM_TURRET) && (ss->current_hits > 0)) {
				...
				if (esct_p->check_all_turrets || (ss->turret_enemy_objnum == esct_p->attacked_objnum)) {
					...
					if (!esct_p->turret_attacking_target ||
					    (esct_p->turret_attacking_target && (ss->turret_enemy_objnum == esct_p->attacked_objnum))) {
						vec3d gsubpos;
	...
	if (!targeting_player && !turret_is_attacking) {
		...
		if ((Game_mode & GM_NORMAL) && (aip->mode != AIM_CHASE) && (aip->mode != AIM_STRAFE) &&
		    (aip->mode != AIM_EVADE) && (aip->mode != AIM_EVADE_WEAPON) && (aip->mode != AIM_AVOID)) {
			return false;
		}
	}
...
int hud_target_closest(int team_mask, int attacked_objnum, int play_fail_snd, int filter,
...
void hud_update_closest_turret()
{
	...
	/*
	   ...
	   core::Assert(Ship_info[shipp->ship_info_index].flags & (SIF_BIG_SHIP|SIF_HUGE_SHIP));
...
void hud_target_subsystem_in_reticle()
{
	...
	for (subsys = GET_FIRST(&Ships[shipnum].subsys_list); subsys != END_OF_LIST(&Ships[shipnum].subsys_list);
...
HudGaugeReticleTriangle::HudGaugeReticleTriangle()
    : HudGauge(HUD_OBJECT_HOSTILE_TRI, HUD_HOSTILE_TRIANGLE, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}

HudGaugeReticleTriangle::HudGaugeReticleTriangle(int _gauge_object, int _gauge_config)
    : HudGauge(_gauge_object, _gauge_config, true, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}

void HudGaugeReticleTriangle::initRadius(int length) { Radius = length; }

void HudGaugeReticleTriangle::initTriBase(float length) { Target_triangle_base = length; }

void HudGaugeReticleTriangle::initTriHeight(float h) { Target_triangle_height = h; }
...
void HudGaugeReticleTriangle::renderTriangleMissileTail(float ang, float xpos, float ypos, float cur_dist,
                                                        int draw_solid, int draw_inside)
{
	float x1    = 0.0f;
	float x2    = 0.0f;
	float y1    = 0.0f;
...
void hud_process_homing_missiles()
{
	...
	if (nearest_dist < Homing_beep.max_cycle_dist) {
		...
		cycle_time =
		    Homing_beep.min_cycle_time + (nearest_dist - Homing_beep.min_cycle_dist) * Homing_beep.precalced_interp;
		...
		if ((delta_time * 1000) > cycle_time) {
			...
			if (closest_is_aspect) {
				Homing_beep.snd_handle =
				    snd_play(gamesnd_get_game_sound(ship_get_sound(Player_obj, GameSounds::PROXIMITY_ASPECT_WARNING)));
			} else {
				Homing_beep.snd_handle =
				    snd_play(gamesnd_get_game_sound(ship_get_sound(Player_obj, GameSounds::PROXIMITY_WARNING)));
			}
		}
	}
}
...
HudGaugeOrientationTee::HudGaugeOrientationTee()
    : HudGauge(HUD_OBJECT_ORIENTATION_TEE, HUD_ORIENTATION_TEE, true, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}

void HudGaugeOrientationTee::initRadius(int length) { Radius = length; }
		...
		if ((Player_obj != NULL) && (mobjp->parent_sig == Player_obj->parent_sig)) {
			if (Weapon_info[Weapons[mobjp->instance].weapon_info_index].wi_flags[Weapon::Info_Flags::Remote]) {
				...
				if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
					switch (mobjp->type) {
					case OBJ_WEAPON:
						hud_target_add_display_list(mobjp, &target_point, &mobjp->pos, 0,
						                            iff_get_color(IFF_COLOR_MESSAGE, 1), NULL, TARGET_DISPLAY_DIST);
						break;
...
core::Assert(targetp != NULL);
	...
	if ((Ships[Message_shipnum].flags[Ship::Ship_Flags::Hidden_from_sensors]) ||
	    ((Ships[Message_shipnum].flags[Ship::Ship_Flags::Stealth]) &&
	     ((Ships[Message_shipnum].team != Player_ship->team) ||
	      (Ships[Message_shipnum].flags[Ship::Ship_Flags::Friendly_stealth_invis])))) {
		return;
	}
...
core::Assert(targetp->instance >= 0 && targetp->instance < MAX_SHIPS);
	...
	if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
		hud_target_add_display_list(targetp, &target_point, &targetp->pos, 10, iff_get_color(IFF_COLOR_MESSAGE, 1),
		                            NULL, 0);
		...
		if ((OBJ_INDEX(targetp) != Player_ai->target_objnum) ||
		    (Message_shipnum == Objects[Player_ai->target_objnum].instance)) {
			if (hud_sensors_ok(Player_ship, 0)) {
				hud_target_add_display_list(targetp, &target_point, &targetp->pos, 0,
				                            iff_get_color(IFF_COLOR_MESSAGE, 1), NULL, TARGET_DISPLAY_DIST);
			}
		}
	}
...
core::Assert((set >= 0) && (set < MAX_KEYED_TARGETS));
	...
	core::Assert(targetp->instance >= 0 && targetp->instance < MAX_SHIPS);
		...
		if (!(target_point.flags & PF_OVERFLOW)) { // make sure point projected
			...
			if (OBJ_INDEX(targetp) == Player_ai->target_objnum) {
				hud_target_add_display_list(targetp, &target_point, &targetp->pos, 5,
				                            iff_get_color(IFF_COLOR_SELECTION, 1), NULL, 0);
				HUD_drew_selection_bracket_on_target = 1;
			} else if (Cmdline_targetinfo) { // Backslash -- show the distance and a lead indicator
				hud_target_add_display_list(targetp, &target_point, &targetp->pos, 5,
				                            iff_get_color(IFF_COLOR_SELECTION, 1), NULL,
				                            TARGET_DISPLAY_DIST | TARGET_DISPLAY_LEAD);
			} else {
				hud_target_add_display_list(targetp, &target_point, &targetp->pos, 5,
				                            iff_get_color(IFF_COLOR_SELECTION, 1), NULL, 0);
			}
		...
		if (target_point.codes == 0) {         // target center is not on screen
			...
			if (Cmdline_targetinfo) {
				target_display_flags = TARGET_DISPLAY_DIST | TARGET_DISPLAY_DOTS | TARGET_DISPLAY_SUBSYS |
				                       TARGET_DISPLAY_NAME | TARGET_DISPLAY_CLASS;
			} else {
				target_display_flags = TARGET_DISPLAY_DIST | TARGET_DISPLAY_DOTS | TARGET_DISPLAY_SUBSYS;
			}
...
void hud_show_hostile_triangle()
{
	...
	for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
		...
		if (Ship_info[sp->ship_info_index].class_type > -1 &&
		    !(Ship_types[Ship_info[sp->ship_info_index].class_type]
		          .flags[Ship::Type_Info_Flags::Show_attack_direction])) {
			continue;
		}
...
void HudGaugeHostileTriangle::render(float /*frametime*/)
{
	if (hostile_obj && maybeFlashSexp() != 1) {
		...
		// hud_set_iff_color( TEAM_HOSTILE, 1 );	//	Note: This should really be TEAM_HOSTILE, not opposite of
		// Player_ship->team.
...
void hud_calculate_lead_pos(vec3d* lead_target_pos, vec3d* target_pos, object* targetp, weapon_info* wip,
                            float dist_to_target, vec3d* rel_pos)
{
	...
	if (vm_vec_mag_quick(&target_moving_direction) < 0.1f) { // Find distance!
		*lead_target_pos = *target_pos;
		...
		polish_predicted_target_pos(wip, targetp, target_pos, lead_target_pos, dist_to_target, &last_delta_vector,
		                            1); // Not used:, float time_to_enemy)
...
int hud_get_best_primary_bank(float* range)
{
	...
	core::Assert(bank_to_fire >= 0 && bank_to_fire < swp->num_primary_banks);
	core::Assert(swp->primary_bank_weapons[bank_to_fire] < MAX_WEAPON_TYPES);
...
//
void polish_predicted_target_pos(weapon_info* wip, object* targetp, vec3d* enemy_pos, vec3d* predicted_enemy_pos,
                                 float dist_to_enemy, vec3d* last_delta_vec, int num_polish_steps)
{
	int iteration;
...
HudGaugeLeadIndicator::HudGaugeLeadIndicator()
    : HudGauge(HUD_OBJECT_LEAD, HUD_LEAD_INDICATOR, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}
	...
	if (lead_target_vertex.codes == 0) { // on screen
		...
		if (!(lead_target_vertex.flags & PF_OVERFLOW)) {
			...
			if (Lead_indicator_gauge.first_frame + frame_offset >= 0) {
				...
				unsize(&sx, &sy);
				renderBitmap(Lead_indicator_gauge.first_frame + frame_offset, fl2i(sx - Lead_indicator_half[0]),
...
void HudGaugeLeadIndicator::renderLeadCurrentTarget()
{
	...
	if (targetp->type == OBJ_WEAPON) {
		if (!(Weapon_info[Weapons[targetp->instance].weapon_info_index]
		          .wi_flags[Weapon::Info_Flags::Can_be_targeted])) {
...
// Ideally I'd like to later turn this into something (or make a new function) that would actually WORK with gun
// convergence/normals instead of the existing code (copied from above) that does some calculations and then is ignored
// ;-) (Go look, what's it actually DO with source_pos?) And also, something that could be called for multiple weapons,
// ITTS style.
...
HudGaugeLeadSight::HudGaugeLeadSight()
    : HudGauge(HUD_OBJECT_LEAD_SIGHT, HUD_LEAD_INDICATOR, true, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}
...
void HudGaugeLeadSight::renderSight(int frame_offset, vec3d* target_pos, vec3d* lead_target_pos)
{
	...
	if (Lead_sight.first_frame >= 0) {
		...
		setGaugeColor();
		renderBitmap(Lead_sight.first_frame + frame_offset, fl2i(reticle_target_sx) + fl2i(HUD_offset_x),
		             fl2i(reticle_target_sy) + fl2i(HUD_offset_y));
	}
}

void HudGaugeLeadSight::pageIn() { bm_page_in_aabitmap(Lead_sight.first_frame, Lead_sight.num_frames); }
...
HudGaugeTargetTriangle::HudGaugeTargetTriangle() : HudGaugeReticleTriangle(HUD_OBJECT_TARGET_TRI, HUD_TARGET_TRIANGLE)
{
}
...
int hud_sensors_ok(ship* sp, int show_msg)
{
	...
	if ((Game_skill_level == 0) || (Game_mode & GM_DEAD) ||
	    ((Game_mode & GM_MULTIPLAYER) && (Net_player->flags & NETINFO_FLAG_OBSERVER))) {
		return 1;
	}
...
void hud_target_next_list(int hostile, int next_flag, int team_mask, int attacked_objnum, int play_fail_snd, int filter,
                          int get_closest_turret_attacking_player)
{
	...
	if (Player_ai->target_objnum == -1 || timestamp_elapsed(timestamp_val)) {
		hud_target_closest(valid_team_mask, attacked_objnum, play_fail_snd, filter,
		                   get_closest_turret_attacking_player);
		return;
	}
...
HudGaugeAutoTarget::HudGaugeAutoTarget()
    : HudGauge(HUD_OBJECT_AUTO_TARGET, HUD_AUTO_TARGET, false, false,
               (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP), 255, 255, 255)
{
}
...
void HudGaugeAutoTarget::pageIn() { bm_page_in_aabitmap(Toggle_frame.first_frame, Toggle_frame.num_frames); }

HudGaugeAutoSpeed::HudGaugeAutoSpeed()
    : HudGauge(HUD_OBJECT_AUTO_SPEED, HUD_AUTO_SPEED, false, false,
               (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP), 255, 255, 255)
{
}
...
void HudGaugeAutoSpeed::pageIn() { bm_page_in_aabitmap(Toggle_frame.first_frame, Toggle_frame.num_frames); }
...
int hud_target_closest_repair_ship(int goal_objnum)
{
	...
	if (nearest_obj != &obj_used_list) {
		...
		if (goal_objnum == -1) {
			HUD_sourced_printf(HUD_SOURCE_HIDDEN, "%s",
			                   XSTR("No support ships in area.  Use messaging to call one in.", 332));
		}
		rval = 0;
	}
	...
	for (i = 0; i < MAX_TRANSMIT_TARGETS; i++) {
		if (Transmit_target_list[transmit_index].objnum >= 0) {
			...
			if (Player_ai->target_objnum == transmit_objnum) {
				...
				if (Transmit_target_list[transmit_index].objsig ==
...
core::Assert((targeted_objnum >= 0) && (targeted_objnum < MAX_OBJECTS));
...
core::Assert(ship_objnum >= 0 && ship_objnum < MAX_OBJECTS);
...
void hudtarget_page_in()
{
	...
	for (i = 0; i < NUM_WEAPON_GAUGES; i++) {
		bm_page_in_aabitmap(Weapon_gauges[ballistic_hud_index][i].first_frame,
		                    Weapon_gauges[ballistic_hud_index][i].num_frames);
	}
...
void hud_stuff_ship_class(char* ship_class_text, ship* shipp)
{
	...
	if (!*ship_class_text) {
		strcpy(ship_class_text, (Ship_info[shipp->ship_info_index].alt_name[0])
		                            ? Ship_info[shipp->ship_info_index].alt_name
...
HudGaugeCmeasures::HudGaugeCmeasures()
    : HudGauge(HUD_OBJECT_CMEASURES, HUD_CMEASURE_GAUGE, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}
...
void HudGaugeCmeasures::pageIn() { bm_page_in_aabitmap(Cmeasure_gauge.first_frame, Cmeasure_gauge.num_frames); }
...
HudGaugeAfterburner::HudGaugeAfterburner()
    : HudGauge(HUD_OBJECT_AFTERBURNER, HUD_AFTERBURNER_ENERGY, true, false,
               (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP), 255, 255, 255)
{
}

void HudGaugeAfterburner::initEnergyHeight(int h) { Energy_h = h; }
...
core::Assert(Player_ship);
	if (!(Ship_info[Player_ship->ship_info_index].flags[Ship::Info_Flags::Afterburner])) {
		...
		percent_left =
		    Player_ship->afterburner_fuel / Ship_info[Player_ship->ship_info_index].afterburner_fuel_capacity;
	}
...
void HudGaugeAfterburner::pageIn() { bm_page_in_aabitmap(Energy_bar.first_frame, Energy_bar.num_frames); }

HudGaugeWeaponEnergy::HudGaugeWeaponEnergy()
    : HudGauge(HUD_OBJECT_WEAPON_ENERGY, HUD_WEAPONS_ENERGY, true, false,
               (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP), 255, 255, 255)
{
}
...
void HudGaugeWeaponEnergy::initEnergyHeight(int h) { Wenergy_h = h; }
...
void HudGaugeWeaponEnergy::initAlwaysShowText(bool show_text) { Always_show_text = show_text; }

void HudGaugeWeaponEnergy::initMoveText(bool move_text) { Moving_text = move_text; }

void HudGaugeWeaponEnergy::initShowBallistics(bool show_ballistics) { Show_ballistic = show_ballistics; }
...
void HudGaugeWeaponEnergy::initBitmaps(char* fname)
{
	...
	if (Energy_bar.first_frame < 0) {
		...
		if (Energy_bar.num_frames != 4) {
			error_display(0,
			              "The animation '%s' has %d frames but the energy bar requires 4 frames in the animation! "
			              "Ignoring animation...",
...
void HudGaugeWeaponEnergy::pageIn() { bm_page_in_aabitmap(Energy_bar.first_frame, Energy_bar.num_frames); }
...
void HudGaugeWeaponEnergy::render(float /*frametime*/)
{
	...
	if (use_new_gauge) {
		...
		remaining = max_w * ((float)Player_ship->weapon_energy /
		                     (float)Ship_info[Player_ship->ship_info_index].max_weapon_reserve);
		if (remaining > 0) {
			setGaugeColor(HUD_C_BRIGHT);
			for (y = 0; y < 10; y++) {
		...
		for (x = 0; x < Player_ship->weapons.num_primary_banks; x++) {
			...
			if (x == Player_ship->weapons.current_primary_bank ||
			...
			if (gr_screen.max_w_unscaled == 640) {
				strcpy_s(shortened_name,
				         Weapon_info[Player_ship->weapons.primary_bank_weapons[x]].get_display_string());
			...
			// Draw the bar graph
			remaining = (max_w - 4) * ((float)Player_ship->weapons.primary_bank_ammo[x] /
		...
		if (percent_left <= 0.3 || Show_ballistic || Always_show_text) {
			...
			renderString(position[0] + Wenergy_text_offsets[0] + delta_x,
			             position[1] + Wenergy_text_offsets[1] + delta_y, buf);
		}
		...
		if (Show_armed) {
			if (Player_ship->flags[Ship::Ship_Flags::Primary_linked]) {
				...
				for (i = 0; i < Player_ship->weapons.num_primary_banks; i++) {
					...
					renderString(position[0] + Armed_name_offsets[0] - w,
					             position[1] + Armed_name_offsets[1] + Armed_name_h * i, buf);
				}
			} else {
...
HudGaugeWeapons::HudGaugeWeapons()
    : HudGauge(HUD_OBJECT_WEAPONS, HUD_WEAPONS_GAUGE, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP, 255, 255, 255)
{
}
...
core::Assert(Player_obj->type == OBJ_SHIP);
core::Assert(Player_obj->instance >= 0 && Player_obj->instance < MAX_SHIPS);
	...
	renderBitmap(primary_top[ballistic_hud_index].first_frame, position[0] + top_offset_x[ballistic_hud_index],
	             position[1]);

	// render the header of this gauge
	renderString(position[0] + Weapon_header_offsets[ballistic_hud_index][0],
	             position[1] + Weapon_header_offsets[ballistic_hud_index][1], EG_WEAPON_TITLE, XSTR("weapons", 328));
	...
	for (i = 0; i < np; i++) {
		...
		if (i == 1) {
			...
			renderBitmap(primary_middle[ballistic_hud_index].first_frame,
			...
			if (primary_last[ballistic_hud_index].first_frame != -1)
				renderBitmap(primary_last[ballistic_hud_index].first_frame,
				             position[0] + frame_offset_x[ballistic_hud_index], y);
		}
		...
		if (((sw->current_primary_bank == i) && !(Player_ship->flags[Ship::Ship_Flags::Primary_linked])) ||
		    ((Player_ship->flags[Ship::Ship_Flags::Primary_linked]) &&
		     !(Weapon_info[sw->primary_bank_weapons[i]].wi_flags[Weapon::Info_Flags::Nolink]))) {
		...
		if (Weapon_info[sw->primary_bank_weapons[0]].hud_image_index != -1) {
			renderBitmap(Weapon_info[sw->primary_bank_weapons[i]].hud_image_index, position[0] + Weapon_pname_offset_x,
			             name_y);
		} else {
	...
	for (i = 0; i < ns; i++) {
		...
		if (i != 0) {
			renderBitmap(secondary_middle[ballistic_hud_index].first_frame,
			             position[0] + frame_offset_x[ballistic_hud_index], y);
		}
		...
		if (sw->current_secondary_bank == i) {
			...
			if ((sw->secondary_bank_ammo[i] > 0) && (sw->current_secondary_bank >= 0)) {
				...
				if ((ms_till_fire >= 500) && ((wip->fire_wait >= 1) || (ms_till_fire > wip->fire_wait * 1000))) {
					renderPrintf(position[0] + Weapon_sreload_offset_x, name_y, EG_NULL, "%d",
					             (int)std::lround(ms_till_fire / 1000.0f));
					cooldown = (int)std::lround(ms_till_fire / 1000.0f);
				}
			}
		} else {
	...
	renderBitmap(secondary_bottom[ballistic_hud_index].first_frame, position[0] + frame_offset_x[ballistic_hud_index],
	             y);
}
...
void hud_target_add_display_list(object* objp, vertex* target_point, vec3d* target_pos, int correction,
                                 color* bracket_clr, char* name, int flags)
{
...
void hud_target_clear_display_list() { target_display_list.clear(); }
...
void HudGaugeOffscreen::initMaxTriSeperation(float length) { Max_offscreen_tri_seperation = length; }

void HudGaugeOffscreen::initMaxFrontSeperation(float length) { Max_front_seperation = length; }

void HudGaugeOffscreen::initTriBase(float length) { Offscreen_tri_base = length; }

void HudGaugeOffscreen::initTriHeight(float length) { Offscreen_tri_height = length; }
		...
		if (target_display_list[i].target_point.codes != 0) {
			...
			if (target_display_list[i].bracket_clr.red && target_display_list[i].bracket_clr.green &&
				...
				if (target_display_list[i].objp) {
					...
					// no object so this must mean it's a nav point. but for some odd reason someone forgot to include a
					// color.
					gr_set_color_fast(&target_display_list[i].bracket_clr);
				}
			}

			calculatePosition(&target_display_list[i].target_point, &target_display_list[i].target_pos, &coords, &dir,
			                  &half_triangle_sep);
...
void HudGaugeOffscreen::calculatePosition(vertex* target_point, vec3d* tpos, vec2d* outcoords, int* dir,
                                          float* half_triangle_sep)
{
	float xpos, ypos;
...
void HudGaugeOffscreen::renderOffscreenIndicator(vec2d* coords, int dir, float distance, float half_triangle_sep,
                                                 bool draw_solid)
{
	float xpos, ypos;
...
HudGaugeWarheadCount::HudGaugeWarheadCount()
    : HudGauge(HUD_OBJECT_WARHEAD_COUNT, HUD_WEAPONS_GAUGE, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY, 255, 255, 255)
{
}
...
void HudGaugeWarheadCount::initMaxSymbols(int count) { Max_symbols = count; }

void HudGaugeWarheadCount::initMaxColumns(int count) { Max_columns = count; }

void HudGaugeWarheadCount::initTextAlign(int align) { Text_align = align; }

void HudGaugeWarheadCount::pageIn() { bm_page_in_aabitmap(Warhead.first_frame, Warhead.num_frames); }
...
core::Assert(Player_obj->instance >= 0 && Player_obj->instance < MAX_SHIPS);
	...
	for (i = 0; i < ammo; i++) {
		...
		renderBitmap(Warhead.first_frame, position[0] + Warhead_count_offsets[0] + column * delta_x,
		             position[1] + Warhead_count_offsets[1] + delta_y);
	}
}

HudGaugeWeaponList::HudGaugeWeaponList(int _gauge_object)
    : HudGauge(_gauge_object, HUD_WEAPONS_GAUGE, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY, 255, 255, 255)
{
}
...
void HudGaugeWeaponList::initBgFirstOffsetX(int x) { _bg_first_offset_x = x; }

void HudGaugeWeaponList::initBgEntryOffsetX(int x) { _bg_entry_offset_x = x; }

void HudGaugeWeaponList::initBgLastOffsetX(int x) { _bg_last_offset_x = x; }

void HudGaugeWeaponList::initBgLastOffsetY(int y) { _bg_last_offset_y = y; }

void HudGaugeWeaponList::initBgFirstHeight(int h) { _background_first_h = h; }

void HudGaugeWeaponList::initBgEntryHeight(int h) { _background_entry_h = h; }

void HudGaugeWeaponList::initHeaderText(char* text) { strcpy_s(header_text, text); }
...
void HudGaugeWeaponList::initEntryStartY(int y) { _entry_start_y = y; }

void HudGaugeWeaponList::initEntryHeight(int h) { _entry_h = h; }
...
HudGaugePrimaryWeapons::HudGaugePrimaryWeapons() : HudGaugeWeaponList(HUD_OBJECT_PRIMARY_WEAPONS) {}

void HudGaugePrimaryWeapons::initPrimaryLinkOffsetX(int x) { _plink_offset_x = x; }

void HudGaugePrimaryWeapons::initPrimaryNameOffsetX(int x) { _pname_offset_x = x; }

void HudGaugePrimaryWeapons::initPrimaryAmmoOffsetX(int x) { _pammo_offset_x = x; }
...
core::Assert(Player_obj->instance >= 0 && Player_obj->instance < MAX_SHIPS);
	...
	for (i = 0; i < num_primaries; ++i) {
		...
		// either render this primary's image or its name
		if (Weapon_info[sw->primary_bank_weapons[0]].hud_image_index != -1) {
			renderBitmap(Weapon_info[sw->primary_bank_weapons[i]].hud_image_index, position[0] + _pname_offset_x,
			             text_y_offset);
		} else {
...
HudGaugeSecondaryWeapons::HudGaugeSecondaryWeapons() : HudGaugeWeaponList(HUD_OBJECT_SECONDARY_WEAPONS) {}

void HudGaugeSecondaryWeapons::initSecondaryAmmoOffsetX(int x) { _sammo_offset_x = x; }

void HudGaugeSecondaryWeapons::initSecondaryNameOffsetX(int x) { _sname_offset_x = x; }

void HudGaugeSecondaryWeapons::initSecondaryReloadOffsetX(int x) { _sreload_offset_x = x; }

void HudGaugeSecondaryWeapons::initSecondaryLinkedOffsetX(int x) { _slinked_offset_x = x; }

void HudGaugeSecondaryWeapons::initSecondaryUnlinkedOffsetX(int x) { _sunlinked_offset_x = x; }
...
core::Assert(Player_obj->instance >= 0 && Player_obj->instance < MAX_SHIPS);
	...
	for (i = 0; i < num_secondaries; ++i) {
		...
		if (sw->current_secondary_bank == i) {
			// show that this is the current secondary armed
			renderPrintf(position[0] + _sunlinked_offset_x, position[1] + text_y_offset, EG_NULL, "%c",
			             Weapon_link_icon);

			// indicate if this is linked
			if (Player_ship->flags[Ship::Ship_Flags::Secondary_dual_fire]) {
				renderPrintf(position[0] + _slinked_offset_x, position[1] + text_y_offset, EG_NULL, "%c",
				             Weapon_link_icon);
			}
			...
			if (wip->hud_image_index != -1) {
				...
				renderString(position[0] + _sname_offset_x, position[1] + text_y_offset,
				             i ? EG_WEAPON_S1 : EG_WEAPON_S2, weapon_name);
			}
			...
			if ((sw->secondary_bank_ammo[i] > 0) && (sw->current_secondary_bank >= 0)) {
				int ms_till_fire = timestamp_until(sw->next_secondary_fire_stamp[sw->current_secondary_bank]);
				if ((ms_till_fire >= 500) && ((wip->fire_wait >= 1) || (ms_till_fire > wip->fire_wait * 1000))) {
					renderPrintf(position[0] + _sreload_offset_x, position[1] + text_y_offset, EG_NULL, "%d",
					             (int)std::lround(ms_till_fire / 1000.0f));
				}
			}
		} else {
			renderString(position[0] + _sname_offset_x, position[1] + text_y_offset, i ? EG_WEAPON_S1 : EG_WEAPON_S2,
			             weapon_name);
		}
...
HudGaugeHardpoints::HudGaugeHardpoints()
    : HudGauge(HUD_OBJECT_HARDPOINTS, HUD_WEAPONS_GAUGE, false, false,
               VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY, 255, 255, 255)
{
}
...
void HudGaugeHardpoints::initLineWidth(float w) { _line_width = w; }

void HudGaugeHardpoints::initViewDir(int dir) { _view_direction = dir; }
...
void HudGaugeHardpoints::render(float /*frametime*/)
{
	...
	render_info.set_flags(MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_TEXTURING | MR_NO_CULL |
	                      MR_NO_ZBUFFER | MR_STENCIL_WRITE | MR_NO_COLOR_WRITES);
	...
	render_info.set_flags(MR_NO_LIGHTING | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_TEXTURING | MR_NO_ZBUFFER |
	                      MR_NO_CULL | MR_STENCIL_READ);
		...
		for (i = 0; i < swp->num_secondary_banks; i++) {
			...
			if (Weapon_info[swp->secondary_bank_weapons[i]].wi_flags[Weapon::Info_Flags::External_weapon_lnch]) {
				for (k = 0; k < bank->num_slots; k++) {
					...
					// We need to transform the position local to the model to be in "world" space relative to the
					// rendered outline
					vec3d world_position;
					vm_vec_unrotate(&world_position, &bank->pnt[k], &object_orient);

					model_render_immediate(&weapon_render_info,
					                       Weapon_info[swp->secondary_bank_weapons[i]].external_model_num,
					                       &object_orient, &bank->pnt[k]);
				}
			} else {
				...
				for (k = 0; k < bank->num_slots; k++) {
					...
					if (swp->current_secondary_bank == i &&
					    (swp->secondary_next_slot[i] == k ||
					     (swp->secondary_next_slot[i] + 1 == k && sp->flags[Ship::Ship_Flags::Secondary_dual_fire]))) {
					...
					vm_vec_scale_add2(
					    &secondary_weapon_pos, &vmd_z_vector,
					    -(1.0f - sp->secondary_point_reload_pct[i][k]) *
	...
	if (draw_primary_models) {
		for (i = 0; i < swp->num_primary_banks; i++) {
			...
			for (k = 0; k < bank->num_slots; k++) {
				if ((Weapon_info[swp->primary_bank_weapons[i]].external_model_num == -1 ||
				     !sip->draw_primary_models[i])) {
					...
					renderCircle((int)draw_point.screen.xyw.x + position[0], (int)draw_point.screen.xyw.y + position[1],
					             10);
					...
					model_render_immediate(&weapon_render_info,
					                       Weapon_info[swp->primary_bank_weapons[i]].external_model_num, &object_orient,
					                       &world_position);
