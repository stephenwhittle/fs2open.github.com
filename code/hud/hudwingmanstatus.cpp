...
HudGaugeWingmanStatus::HudGaugeWingmanStatus()
    : HudGauge(HUD_OBJECT_WINGMAN_STATUS, HUD_WINGMEN_STATUS, false, false,
...
void HudGaugeWingmanStatus::initFixedHeaderPosition(bool fixed) { fixed_header_position = fixed; }

void HudGaugeWingmanStatus::initLeftFrameEndX(int x) { left_frame_end_x = x; }
...
void HudGaugeWingmanStatus::initWingWidth(int w) { wing_width = w; }

void HudGaugeWingmanStatus::initRightBgOffset(int offset) { right_frame_start_offset = offset; }
...
void HudGaugeWingmanStatus::initBitmaps(char* fname_left, char* fname_middle, char* fname_right, char* fname_dots)
{
	...
	if (Wingman_status_left.first_frame == -1) {
		core::Warning(LOCATION, "Error loading %s\n", fname_left);
	}
...
void HudGaugeWingmanStatus::initGrowMode(int mode) { grow_mode = mode; }
...
void HudGaugeWingmanStatus::renderBackground(int num_wings_to_draw)
{
	...
	// bring us to the end of the left portion so we can draw the last or middle bits depending on how many wings we
	// have to draw
...
{
	return HUD_wingman_status[wing_index].status[wing_pos];
}
