	...
	try
	{
		...
		while (!optional_string("#Storms end")){
			...
			while (optional_string("+bolt:")){
				...
				core::Assert(new_storm_type.bolt_types[new_storm_type.num_bolt_types] != (size_t)-1);
...
core::Assert( lp != &Nebl_free_list );		// shouldn't have the dummy element
...
{		
	...
	if(prev == NULL){
	core::Assert(whee->links[LINK_RIGHT] != NULL);
