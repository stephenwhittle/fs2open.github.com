...
static std::string opengl_load_shader(const char* filename) {
 ...
 core::mprintf("   Loading built-in default shader for: %s\n", filename);
...
static bool load_cached_shader_binary(opengl::ShaderProgram* program, const std::string& hash) {
	...
	if (!metadata_root) {
	 core::mprintf("Loading of cache metadata failed! Falling back to GLSL shader...\n");
		return false;
	}

	json_int_t format;
	if (json_unpack(metadata_root, "{sI}", "format", &format) != 0) {
	 core::mprintf("Failed to unpack values from metadata JSON! Falling back to GLSL shader...\n");
		return false;
	}
...
static void cache_program_binary(GLuint program, const std::string& hash) {
	...
	auto metadata_fp = cfopen(metadata_name.c_str(), "wb", CFILE_NORMAL, CF_TYPE_CACHE, false,
	                          CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME | CF_LOCATION_TYPE_ROOT);
	if (!metadata_fp) {
	 core::mprintf("Could not open shader cache metadata file!\n");
		return;
	}
	...
	if (json_dump_callback(metadata, json_write_callback, metadata_fp, 0) != 0) {
	 core::mprintf("Failed to write shader cache metadata file!\n");
		cfclose(metadata_fp);
		return;
	}
	cfclose(metadata_fp);
	...
	auto binary_fp = cfopen(binary_name.c_str(), "wb", CFILE_NORMAL, CF_TYPE_CACHE, false,
	                        CF_LOCATION_ROOT_USER | CF_LOCATION_ROOT_GAME | CF_LOCATION_TYPE_ROOT);
	if (!binary_fp) {
	 core::mprintf("Could not open shader cache binary file!\n");
		return;
	}
...
core::Assert(sdr_info->type_id == sdr);
 core::mprintf("Compiling new shader:\n");
 ...
 core::mprintf("Shader Variant Features:\n");
...
void opengl_shader_init()
{
 ...
 core::mprintf("Compiling passthrough shader...\n");
