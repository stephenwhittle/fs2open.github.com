...
int gr_opengl_tcache_set_internal(int bitmap_handle, int bitmap_type, float *u_scale, float *v_scale, uint32_t *array_index, int tex_unit = 0)
{
	...
	tcache_slot_opengl *t = bm_get_gr_info<tcache_slot_opengl>(bitmap_handle, true);
...
{
	tcache_slot_opengl *ts = bm_get_gr_info<tcache_slot_opengl>(slot);
...
void gr_opengl_update_texture(int bitmap_handle, int bpp, const ubyte* data, int width, int height)
{
	...
	tcache_slot_opengl *t = bm_get_gr_info<tcache_slot_opengl>(bitmap_handle);
	if(!t->texture_id)
		return;
	int byte_mult = (bpp >> 3);
...
struct fbo_t {
	...
	fbo_t() :
		...
		working_handle(-1), is_static(0), fbo_id(-1)
	{
	}
};
...
void opengl_kill_render_target(bitmap_slot* slot)
{
	...
	tcache_slot_opengl *ts = static_cast<tcache_slot_opengl*>(slot->gr_info);
...
int opengl_set_render_target( int slot, int face, int is_static )
{
	...
	if (slot < 0) {
		if ( (render_target != NULL) && (render_target->working_handle >= 0) ) {
...
int opengl_make_render_target( int handle, int *w, int *h, int *bpp, int *mm_lvl, int flags )
{
	...
	tcache_slot_opengl *ts = bm_get_gr_info<tcache_slot_opengl>(handle);
