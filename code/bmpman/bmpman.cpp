...
{
	return bm_is_anim(bm_get_entry(num));
}
...
static void allocate_new_block() {
	...
	for (size_t i = 0; i < new_block.size(); ++i) {
		auto& slot = new_block[i];
		auto& entry = slot.entry;
		...
		entry.info.user.data = NULL;
		entry.mem_taken = 0;
		entry.bm.data = 0;
		entry.bm.palette = NULL;
...
*/
static int bm_load_image_data(const char *filename, int handle, int bpp, ubyte flags, bool nodebug)
{
	BM_TYPE c_type = BM_TYPE_NONE;
	int true_bpp;
...
int bm_load_animation(const char *real_filename, int *nframes, int *fps, int *keyframe, float *total_time, bool can_drop_frames, int dir_type) {
	...
	for (i = 0; i < anim_frames; i++) {
		...
		if (type == BM_TYPE_EFF) {
			...
			if (bm_load_info(eff_type, n + i, entry->info.ani.eff.filename, NULL, &anim_width, &anim_height, &bpp, &c_type, &mm_lvl, &img_size)) {
...
bitmap * bm_lock(int handle, int bpp, ubyte flags, bool nodebug) {
	...
	// read the file data
	if (bm_load_image_data(be->filename, handle, bpp, flags, nodebug) == -1) {
...
void bm_lock_ani(int handle, bitmap_slot *bs, bitmap *bmp, int bpp, ubyte flags) {
	...
	if ((the_anim = anim_load(first_entry->filename, first_entry->dir_type)) == NULL) {
		...
		for (auto& slot : block) {
			...
			if ((entry.type != BM_TYPE_NONE) && (entry.type != BM_TYPE_RENDER_TARGET_DYNAMIC)
				...
				if (entry.preloaded) {
					...
					if ((entry.info.ani.first_frame == 0) || (entry.info.ani.first_frame == i)) {
#ifndef NDEBUG
