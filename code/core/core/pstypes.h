...
//moved from physics.h - should possibly go into its own lib or something. not sure if a header full of all the typedefs is the best way to go
// information for physics sim for an object
typedef struct physics_info {
	uint flags; // misc physics flags

	float mass;           // the mass of this object
	vec3d center_of_mass; // Goober5000 - this is never ever used by physics; currently physics assumes the center of an
	                      // object is the center of mass
	...
	float glide_cap;         // Backslash - for 'newtonian'-style gliding, the cap on velocity (so that something can't
	                         // accelerate to ridiculous speeds... unless allowed to)
	float cur_glide_cap;     // SUSHI: Used for dynamic glide cap, so we can use the ramping function on the glide cap
	float glide_accel_mult;  // SUSHI: The acceleration multiplier for glide mode. A value < 0 means use glide ramping
	                         // instead
