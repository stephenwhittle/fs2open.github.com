...
static const char* Game_title_screen_fname[GR_NUM_RESOLUTIONS] = {"PreLoad", "2_PreLoad"};

static const char* Game_logo_screen_fname[GR_NUM_RESOLUTIONS] = {"PreLoadLogo", "2_PreLoadLogo"};
...
void game_sunspot_process(float frametime)
{
	...
	if (sn_stage)
	{
		...
		switch (sn_stage)
		{
			...
			// camera cut. player not in control. note : at this point camera starts out facing the sun. so we can go
			// nice and bright
			case 2:
			case 3:
				Sun_spot_goal = 0.9f;
	...
	if (time > -duration)
	{
		...
		else
		{
			...
			Big_expl_flash.cur_flash_intensity =
				Big_expl_flash.max_flash_intensity * ((float) TIME_DOWN - time) / (float) TIME_DOWN;
		}
	}
...
void game_level_init()
{
	...
	// reset the geometry map and distortion map batcher, this should to be done pretty soon in this mission load
	// process (though it's not required)
	batch_reset();
...
void game_loading_callback(int count)
{
	...
	Assertion(Game_loading_ani.num_frames > 0,
			  "Load Screen animation %s not found, or corrupted. Needs to be an animation with at least 1 frame.",
			  Game_loading_ani.filename);

	int do_flip = 0;

	int new_framenum =
		bm_get_anim_frame(Game_loading_ani.first_frame, static_cast<float>(count), static_cast<float>(COUNT_ESTIMATE));
...
void game_post_level_init()
{
	...
	gr_set_ambient_light(The_mission.ambient_light_level & 0xff, (The_mission.ambient_light_level >> 8) & 0xff,
...
DCF(show_cpu, "Toggles showing cpu usage")
{
	...
	if (dc_optional_string_either("help", "--help"))
	{
		dc_printf("Usage: (optional) bool Show_cpu\n If true, Show_cpu is set and Show_mem is cleared.  If false, then "
				  "Show_cpu is cleared.  If nothing passed, then toggle.\n");
		process = false;
	}
...
void game_init()
{
	...
	auto GameSettingsHandle = SCPModuleManager::GetModule<SCP::GameSettingsModule>();
	if (!GameSettingsHandle)
	{
		return;
	}
	// This needs to be delayed until we know if the new
	// options are actually going to be used
	if (GameSettingsHandle->GameSettings()->OtherSettings->EnableIngameOptions)
	{
	...
	if (CmdlineModule->CurrentOptions->bStandalone)
	{
...
// Karajorma - Moved here from the sound init code cause otherwise windows complains
	...
	if (Cmdline_voice_recognition)
	{
		...
		if (SDL_GetWindowWMInfo(os::getSDLMainWindow(), &info))
...
void game_show_framerate()
{
		...
		for (pss = GET_FIRST(&shipp->subsys_list); pss != END_OF_LIST(&shipp->subsys_list); pss = GET_NEXT(pss))
		{
			if (pss->system_info->type == SUBSYSTEM_TURRET)
			{
				if (pss->turret_enemy_objnum == -1)
					gr_printf_no_resize(gr_screen.center_offset_x + 10, gr_screen.center_offset_y + (t * line_height),
										"Turret %d: <None>", t);
				else if (Objects[pss->turret_enemy_objnum].type == OBJ_SHIP)
					gr_printf_no_resize(gr_screen.center_offset_x + 10, gr_screen.center_offset_y + (t * line_height),
										"Turret %d: %s", t,
										Ships[Objects[pss->turret_enemy_objnum].instance].ship_name);
				else
					gr_printf_no_resize(gr_screen.center_offset_x + 10, gr_screen.center_offset_y + (t * line_height),
	...
	if ((Show_framerate && HUD_draw) || Cmdline_frame_profile || Cmdline_bmpman_usage)
	{
		...
		{
			if (frametotal != 0.0f)
				gr_printf_no_resize(gr_screen.center_offset_x + 20, gr_screen.center_offset_y + 100, "FPS: %0.1f",
									Framerate);
			else
		...
		{
			gr_printf_no_resize(gr_screen.center_offset_x + 20, gr_screen.center_offset_y + 100 - line_height,
								"BMPMAN: %d/%d", bmpman_count_bitmaps(), bmpman_count_available_slots());
		}
	}
	...
	if (Cmdline_show_stats && HUD_draw)
	{
		...
		if (GetProcessMemoryInfo(GetCurrentProcess(), reinterpret_cast<PPROCESS_MEMORY_COUNTERS>(&process_stats),
								 sizeof(process_stats)))
		{
			sprintf(mem_buffer, "Private Usage: " SIZE_T_ARG " Meg",
					static_cast<size_t>(process_stats.PrivateUsage) / 1024 / 1024);
			gr_string(sx, sy, mem_buffer.c_str(), GR_RESIZE_NONE);
			sy += line_height;

			sprintf(mem_buffer, "Working set size: " SIZE_T_ARG " Meg",
					static_cast<size_t>(process_stats.WorkingSetSize) / 1024 / 1024);
			gr_string(sx, sy, mem_buffer.c_str(), GR_RESIZE_NONE);
			sy += line_height;
			sy += line_height;
		}
		...
		{
			sprintf(mem_buffer, "Physical Free: %" PRIu64 " / %" PRIu64 " Meg", mem_stats.ullAvailPhys / 1024 / 1024,
					mem_stats.ullTotalPhys / 1024 / 1024);
			gr_string(sx, sy, mem_buffer.c_str(), GR_RESIZE_NONE);
			sy += line_height;

			sprintf(mem_buffer, "Pagefile Free: %" PRIu64 " / %" PRIu64 " Meg",
					mem_stats.ullAvailPageFile / 1024 / 1024, mem_stats.ullTotalPageFile / 1024 / 1024);
			gr_string(sx, sy, mem_buffer.c_str(), GR_RESIZE_NONE);
			sy += line_height;

			sprintf(mem_buffer, "Virtual Free:  %" PRIu64 " / %" PRIu64 " Meg", mem_stats.ullAvailVirtual / 1024 / 1024,
	...
	if (Show_cpu == 1)
	{
		...
		if (Timing_total > 0.01f)
		{
			...
			sy += line_height;
			gr_printf_no_resize(sx, sy, NOX("GAME: %.0f%%"),
								(Timing_total - (Timing_render2 + Timing_render3 + Timing_flip + Timing_clear)) *
									100.0f / Timing_total);
			sy += line_height;
		}
	}
	...
	if (Show_player_pos)
	{
		...
		gr_printf_no_resize(sx, sy, NOX("Player Pos: (%d,%d,%d)"), fl2i(Player_obj->pos.xyz.x),
							fl2i(Player_obj->pos.xyz.y), fl2i(Player_obj->pos.xyz.z));
	}
...
void game_show_eye_pos(camid cid)
{
	...
	// Position
	gr_printf_no_resize(gr_screen.center_offset_x + 20, gr_screen.center_offset_y + 100 - font_height, "X:%f Y:%f Z:%f",
	...
	rot_angles.b *= (180 / PI);
	rot_angles.h *= (180 / PI);
	gr_printf_no_resize(gr_screen.center_offset_x + 20, gr_screen.center_offset_y + 100 - font_height,
						"Xr:%f Yr:%f Zr:%f", rot_angles.p, rot_angles.b, rot_angles.h);
}
...
void game_show_time_left()
{
	...
	gr_printf_no_resize(gr_screen.center_offset_x + 5, gr_screen.center_offset_y + 40,
...
DCF(single_step, "Enables single step mode.")
{
	...
	{
		dc_printf(
			"Usage: game_single_step [bool]\nEnables or disables single-step mode.  If nothing passed, then toggles "
...
void game_tst_frame_pre()
{
	...
	if (tst == 3)
	{
		...
		// offscreen
		if (!((v.screen.xyw.x >= 0) && (v.screen.xyw.x <= gr_screen.max_w) && (v.screen.xyw.y >= 0) &&
		...
		else
		{
			...
			if ((vm_vec_dist_quick(&tst_pos, &Eye_position) <= 300.0f) &&
				((tst_time == 0) || ((time(nullptr) - tst_time) >= 10)))
			{
				tst = 2;
			}
		}
...
void game_tst_mark(object* objp, ship* shipp)
{
	...
	// bogus
	if ((objp == nullptr) || (shipp == nullptr) || (shipp->ship_info_index < 0) ||
		(shipp->ship_info_index >= ship_info_size()))
	{
		return;
	}
...
void player_repair_frame(float frametime)
{
	if (MULTIPLAYER_MASTER)
	{
		...
		{
			if (MULTI_CONNECTED(np) && (Net_player != nullptr) && (Net_player->player_id != np.player_id) &&
				(np.m_player != nullptr) && (np.m_player->objnum >= 0) && (np.m_player->objnum < MAX_OBJECTS))
			{
				// don't rearm/repair if the player is dead or dying/departing
				if (!NETPLAYER_IS_DEAD((&np)) &&
					!(Ships[Objects[np.m_player->objnum].instance].is_dying_or_departing()))
				{
					ai_do_repair_frame(&Objects[np.m_player->objnum],
									   &Ai_info[Ships[Objects[np.m_player->objnum].instance].ai_index], frametime);
				}
			}
		}
	}
	...
	if ((Player_obj != nullptr) && (Player_obj->type == OBJ_SHIP) && !(Game_mode & GM_STANDALONE_SERVER) &&
		(Player_ship != nullptr) && !(Player_ship->flags[Ship::Ship_Flags::Dying]))
	{
...
void say_view_target()
{
	...
	if (!(Game_mode & GM_DEAD_DIED) &&
		((Game_mode & (GM_DEAD_BLEW_UP)) || ((Last_view_target != nullptr) && (Last_view_target != view_target))))
	{
		if (view_target != Player_obj)
		{
			...
			switch (Objects[Player_ai->target_objnum].type)
			{
				case OBJ_SHIP:
					...
					{
						strcpy_s(view_target_name,
								 Ships[Objects[Player_ai->target_objnum].instance].get_display_string());
					}
					break;
				case OBJ_WEAPON:
					strcpy_s(view_target_name,
							 Weapon_info[Weapons[Objects[Player_ai->target_objnum].instance].weapon_info_index]
								 .get_display_string());
			...
			if (strlen(view_target_name))
			{
				...
				HUD_fixed_printf(0.0f, gr_screen.current_color, XSTR("Viewing %s%s\n", 185),
								 (Viewer_mode & VM_OTHER_SHIP) ? XSTR("from ", 186) : "", view_target_name);
...
void apply_view_shake(matrix* eye_orient)
{
	...
	if (Viewer_obj == Player_obj)
	{
		...
		if (Player_obj->type == OBJ_SHIP && (Ships[Player_obj->instance].wash_intensity > 0) && Wash_on)
		{
			...
			joy_ff_play_dir_effect(FF_SCALE * wash_intensity * rand_vec.xyz.x,
		...
		if (Game_shudder_time != -1)
		{
			...
			{
				tangles.p +=
					get_shake(Game_shudder_intensity * 0.005f, timestamp_until(Game_shudder_time), Game_shudder_total);
				tangles.h +=
					get_shake(Game_shudder_intensity * 0.005f, timestamp_until(Game_shudder_time), Game_shudder_total);
			}
		}
...
camid game_render_frame_setup()
{
	...
	if ((!(last_Viewer_mode & VM_EXTERNAL) && (Viewer_mode & VM_EXTERNAL)) || // internal to external
		...
		((last_Viewer_mode & VM_TOPDOWN) && !(Viewer_mode & VM_TOPDOWN)) || (fov_changed) ||
		((Viewer_mode & VM_OTHER_SHIP) &&
		 (last_Viewer_objnum != Player_ai->target_objnum)) // other ship mode, but targets changes
	)
	{
	...
	if ((last_Viewer_mode != Viewer_mode) || (last_Game_mode != Game_mode) || (fov_changed) ||
	...
	if (Game_mode & GM_DEAD)
	{
		...
		else
		{
			...
			if (!(Game_mode & GM_DEAD_BLEW_UP))
			{
				...
				// nprintf(("AI", "Player death roll vel = %7.3f %7.3f %7.3f\n", Player_obj->phys_info.vel.x,
				// Player_obj->phys_info.vel.y, Player_obj->phys_info.vel.z));
			}
			...
			else
			{
				...
				vm_vec_scale_add2(&Dead_camera_pos, &Original_vec_to_deader,
								  MIN(25.0f, vm_vec_mag_quick(&Dead_player_last_vel)) * flFrametime);
			}
	...
	else
	{
		...
		if (!(Game_mode & (GM_DEAD | GM_DEAD_BLEW_UP)))
		{
			...
			else if (Viewer_mode & VM_EXTERNAL)
			{
				...
				vm_vec_scale_add(&eye_pos, &Viewer_obj->pos, &tm.vec.fvec,
			...
			else if (Viewer_mode & VM_CHASE)
			{
				...
				if (Viewer_obj == Player_obj)
				{
					...
					vm_vec_scale_add2(&eye_pos, &eyemat.vec.fvec,
									  -2.125f * Viewer_obj->radius - Viewer_chase_info.distance);
					vm_vec_scale_add2(&eye_pos, &eyemat.vec.uvec,
									  0.625f * Viewer_obj->radius + 0.35f * Viewer_chase_info.distance);
				...
				else
				{
					vm_vec_scale_add(&eye_pos, &Viewer_obj->pos, &move_dir, -0.02f * Viewer_obj->radius);
					vm_vec_scale_add2(&eye_pos, &eyemat.vec.fvec,
									  -2.5f * Viewer_obj->radius - Viewer_chase_info.distance);
					vm_vec_scale_add2(&eye_pos, &eyemat.vec.uvec,
									  0.75f * Viewer_obj->radius + 0.35f * Viewer_chase_info.distance);
					vm_vec_sub(&tmp_dir, &Viewer_obj->pos, &eye_pos);
					vm_vec_normalize(&tmp_dir);
				}
			...
			else
			{
				...
				switch (Viewer_obj->type)
				{
					...
					case OBJ_OBSERVER:
						...
						break;
					default:
						mprintf(("Invalid Value for Viewer_obj->type. Expected values are OBJ_SHIP (1) and "
								 "OBJ_OBSERVER (12), we encountered %d. Please tell a coder.\n",
								 Viewer_obj->type));
						Int3();
				}
			}
...
void game_simulation_frame()
{
	...
	if (MULTIPLAYER_MASTER && (Net_player != nullptr) && (Netgame.type_flags & NG_TYPE_DOGFIGHT) &&
		(f2fl(Missiontime) >= 2.0f) && !dogfight_blown)
	{
		...
		{
			// bogus
			if ((moveup->objnum < 0) || (moveup->objnum >= MAX_OBJECTS) || (Objects[moveup->objnum].type != OBJ_SHIP) ||
				(Objects[moveup->objnum].instance < 0) || (Objects[moveup->objnum].instance >= MAX_SHIPS) ||
				(Ships[Objects[moveup->objnum].instance].ship_info_index < 0) ||
				(Ships[Objects[moveup->objnum].instance].ship_info_index >= ship_info_size()))
			{
			...
			if ((sip->is_small_ship()) && (multi_find_player_by_object(&Objects[moveup->objnum]) < 0) &&
	...
	// the basic idea being that because it con be confusing to deal with them on a multi-frame basis, they are only
	// valid for frame
	...
	// always check training objectives, even in multiplayer missions. we need to do this so that the directives gauge
	// works properly on clients
	...
	if (Fade_type != FI_NONE)
	{
		...
		if (timestamp() >= Fade_start_timestamp)
		{
			...
			if (timestamp() < Fade_end_timestamp)
			{
				...
				alpha = fl2i((float) startAlpha +
							 (((float) endAlpha - (float) startAlpha) / (float) duration) * (float) elapsed);
			}
			else
			{
				// Fade finished
	...
	if ((Cutscene_bar_flags & CUB_GRADUAL) && Cutscene_bars_progress < 1.0f)
	{
		...
		if (Cutscene_bar_flags & CUB_CUTSCENE)
			...
			yborder = gr_screen.max_h / CUTSCENE_BAR_DIVISOR -
					  fl2i(Cutscene_bars_progress * (gr_screen.max_h / CUTSCENE_BAR_DIVISOR));
...
void game_frame(bool paused)
{
	...
	if (!Pre_player_entry)
	{
		if (!(Game_mode & GM_STANDALONE_SERVER))
		{
			...
			// (this is approximately where the red_alert_check_status() function tree began in the pre-HUD-overhaul
			// code)
...
void game_set_frametime(int state)
{
	...
	if ((Game_mode & GM_STANDALONE_SERVER) && (f2fl(Frametime) < ((float) 1.0 / (float) Multi_options_g.std_framecap)))
	{
...
#ifndef NDEBUG
		mprintf(
			("Frame %2i too long!!: frametime = %.3f (%.3f)\n", Framecount, f2fl(Frametime), f2fl(debug_frametime)));
#endif
...
int game_poll()
{
	...
	switch (k)
	{
		...
		case KEY_F2:
			...
			if ((Game_mode & GM_MULTIPLAYER) && (Net_player != nullptr) &&
			...
			switch (state)
			{
				...
				case GS_STATE_DEATH_DIED:
					//				case GS_STATE_DEATH_BLEW_UP:	// 	DEATH_BLEW_UP might be okay but do not comment out
					//DEATH_DIED as otherwise no clean up is performed on the dead ship
		...
		case KEY_F3:
			if ((state == GS_STATE_TEAM_SELECT) || (state == GS_STATE_BRIEFING) || (state == GS_STATE_SHIP_SELECT) ||
				(state == GS_STATE_WEAPON_SELECT) || (state == GS_STATE_GAME_PLAY) || (state == GS_STATE_GAME_PAUSED))
			{
		...
		case KEY_F4:
			...
			else
			{
				if ((state == GS_STATE_GAME_PLAY) || (state == GS_STATE_DEATH_DIED) ||
					(state == GS_STATE_DEATH_BLEW_UP) || (state == GS_STATE_GAME_PAUSED))
				{
					gameseq_post_event(GS_EVENT_MISSION_LOG_SCROLLBACK);
					k = 0;
				}
...
{
	mprintf(("Got event %s (%d) in state %s (%d)\n", GS_event_text[event], event, GS_state_text[current_state],
			 current_state));
	...
	switch (event)
	{
		...
		case GS_EVENT_DEBRIEF:
			...
			if (Campaign_ending_via_supernova &&
				(Game_mode & GM_CAMPAIGN_MODE) /* && !stricmp(Campaign.filename, "freespace2")*/)
			{
		...
		case GS_EVENT_END_GAME:
			if ((current_state == GS_STATE_GAME_PLAY) || (current_state == GS_STATE_DEATH_DIED) ||
				(current_state == GS_STATE_DEATH_BLEW_UP) || (current_state == GS_STATE_DEBRIEF) ||
		...
		case GS_EVENT_HOTKEY_SCREEN:
			gameseq_push_state(GS_STATE_HOTKEY_SCREEN); // use push_state() since we might get to this screen through a
														// variety of states
			break;
...
void game_leave_state(int old_state, int new_state)
{
	...
	switch (old_state)
	{
		...
		case GS_STATE_SHIP_SELECT:
			if (new_state != GS_STATE_OPTIONS_MENU && new_state != GS_STATE_WEAPON_SELECT &&
				new_state != GS_STATE_HOTKEY_SCREEN && new_state != GS_STATE_BRIEFING &&
				new_state != GS_STATE_TEAM_SELECT && (new_state != GS_STATE_MULTI_MISSION_SYNC))
			{
				common_select_close();
				if (new_state == GS_STATE_MAIN_MENU)
		...
		case GS_STATE_TEAM_SELECT:
			if (new_state != GS_STATE_OPTIONS_MENU && new_state != GS_STATE_SHIP_SELECT &&
				new_state != GS_STATE_HOTKEY_SCREEN && new_state != GS_STATE_BRIEFING &&
				new_state != GS_STATE_WEAPON_SELECT && (new_state != GS_STATE_MULTI_MISSION_SYNC))
			{
				common_select_close();
				if (new_state == GS_STATE_MAIN_MENU)
				{
...
void game_enter_state(int old_state, int new_state)
{
	...
	switch (new_state)
	{
		case GS_STATE_MAIN_MENU:
			...
			if ((Cmdline_start_netgame || (Cmdline_connect_addr != nullptr)) &&
				(!Main_hall_netgame_started) /*&& (Game_mode == GM_MULTIPLAYER)*/)
			{ // DTP added "&& (game_mode == GM_multiplayer)" so that ppl don't get thrown into Multiplayer with a
			  // Singleplayer Pilot.
		...
		case GS_STATE_GAME_PLAY:
			...
			if ((old_state == GS_STATE_GAME_PLAY) || (old_state == GS_STATE_BRIEFING) ||
				(old_state == GS_STATE_DEBRIEF) || (old_state == GS_STATE_SHIP_SELECT) ||
				(old_state == GS_STATE_WEAPON_SELECT) || (old_state == GS_STATE_RED_ALERT))
			{
			...
			if ((Game_mode & GM_NORMAL) && ((old_state == GS_STATE_MAIN_MENU) || (old_state == GS_STATE_GAME_PLAY) ||
			...
			if (!(Game_mode & GM_MULTIPLAYER) &&
				((old_state == GS_STATE_BRIEFING) || (old_state == GS_STATE_SHIP_SELECT) ||
				 (old_state == GS_STATE_WEAPON_SELECT) || (old_state == GS_STATE_MAIN_MENU) ||
				 (old_state == GS_STATE_MULTI_STD_WAIT) || (old_state == GS_STATE_SIMULATOR_ROOM)))
			{
...
// XSTR:ON
			...
			if (!(Game_mode & GM_MULTIPLAYER) &&
				((old_state == GS_STATE_MAIN_MENU) || (old_state == GS_STATE_DEATH_BLEW_UP) ||
			...
			if (!(Game_mode & GM_STANDALONE_SERVER) &&
				((old_state != GS_STATE_GAME_PAUSED) && (old_state != GS_STATE_MULTI_PAUSED)))
			{
				event_music_first_pattern(); // start the first pattern
			}
...
int game_do_ram_check(uint64_t ram_in_bytes)
{
	if (ram_in_bytes < 30 * 1024 * 1024)
	{
		...
		if (allowed_to_run)
		{
			sprintf(
				tmp,
				XSTR("FreeSpace has detected that you only have %dMB of free memory.\n\nFreeSpace requires at least "
					 "32MB of memory to run.  If you think you have more than %dMB of physical memory, ensure that you "
					 "aren't running SmartDrive (SMARTDRV.EXE).  Any memory allocated to SmartDrive is not usable by "
		...
		{
			sprintf(tmp,
					XSTR("FreeSpace has detected that you only have %dMB of free memory.\n\nFreeSpace requires at "
						 "least 32MB of memory to run.  If you think you have more than %dMB of physical memory, "
						 "ensure that you aren't running SmartDrive (SMARTDRV.EXE).  Any memory allocated to "
...
#define BAIL()                                   \
	...
	{                                            \
		...
		{                                        \
			if (pof_list[_idx] != nullptr)       \
			{                                    \
				vm_free(pof_list[_idx]);         \
				pof_list[_idx] = nullptr;        \
			}                                    \
		}                                        \
		return;                                  \
	} while (false);
...
int game_main(int argc, char* argv[])
{
	...
	if (!tmp_mem)
	{
		os::dialogs::Message(
			...
			XSTR("Not enough memory to run FreeSpace.\r\nTry closing down some other applications.\r\n", 198),
...
void game_do_training_checks()
{
	...
	if ((Players_target == UNINITIALIZED) || (Player_ai->target_objnum != Players_target) ||
...
void game_show_event_debug(float /*frametime*/)
{
	...
	gr_string((gr_screen.clip_width_unscaled - font_width) / 2, gr_screen.center_offset_y + 15,
			  NOX("EVENT DEBUG VIEW"));
	...
	while (k < ED_count)
	{
		...
		if (z & EVENT_DEBUG_EVENT)
		{
			...
			sprintf(buf, NOX("%s%s (%s) %s%d %d"), (Mission_events[z].flags & MEF_CURRENT) ? NOX("* ") : "",
					...
					(Mission_events[z].chain_delay < 0) ? "" : NOX("x "), Mission_events[z].repeat_count,
					Mission_events[z].interval);
		}
		else
		{
			i = (z >> 16) * 3;
...
void Time_model(int modelnum)
{
	...
	fprintf(Time_fp, "\"%s\"\t%.0f\t%d\t%d\t%d\t%d\n", pof_file, i2fl(framecount) / f2fl(t2 - t1),
			bitmaps_used_this_frame, modelstats_num_polys, modelstats_num_verts, Tmap_npixels);
}
...
void game_feature_disabled_popup()
{
	popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK,
		  XSTR("Sorry, the requested feature is currently disabled in this build", 1621));
}
...
void game_start_subspace_ambient_sound()
{
	...
	{
		Subspace_ambient_left_channel =
			snd_play_looping(gamesnd_get_game_sound(GameSounds::SUBSPACE_LEFT_CHANNEL), -1.0f);
	}

	if (!Subspace_ambient_right_channel.isValid())
	{
		Subspace_ambient_right_channel =
			snd_play_looping(gamesnd_get_game_sound(GameSounds::SUBSPACE_RIGHT_CHANNEL), 1.0f);
	}
}
